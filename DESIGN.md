...
- Agent detail is a three-column fixed-role workbench at 1440×900: profile/settings (fixed Codex provider, model, reasoning and Skills), one large Developer instructions Markdown editor without Preview, and a role-specific purpose/workflow/safety guide. The only toolbar mutation is `Save Agent`; create/delete/execution commands do not exist. Missing or invalid TOML remains navigable and names its exact Git restoration path. At 390×844 the columns stack in that reading order with no page-level horizontal overflow.
- Action Workspace labels only `Validation Agent` and `Work Agent`; generated agent IDs, permissions and TOML descriptions remain canonical but are not rendered or edited there. Each role edits developer instructions, model, reasoning and Skills. Model uses compact radio cards, reasoning uses a labelled discrete slider on the same settings row at workbench width, and selected Skills use removable shadcn badges with a ghost `+ Skill` chooser. `Save Action` persists Project Config and both Action-agent TOMLs atomically with optimistic hashes from the form-end save row and is disabled during an active Run. Positive readiness prose is omitted; blocking issues remain explicit beside the disabled save. There is no instruction dropdown or machine-local execution editor. Provider, sandbox, network, read-only-root, Computer and pairing controls do not exist.
