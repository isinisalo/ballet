...
- **Flow Visualization:** Use the three-level Loop Engineer hierarchy. Context is a read-only Project intent → Ballet Loop system → declared outcomes planet system built from the tracked Loop artwork, grid, orbit lines, exact counts, and text labels. Level 1 · Loops uses one compact rectangular black box per `ProjectLoop`; arrange larger systems in a deterministic three-column snake that uses canvas height without shrinking labels into an unreadable single row. Always render every `flow` Loop Edge, then show only the selected Loop's incident `repair` Loop Edges and an exact hidden-repair count. Label every visible Edge with icon and text, and keep the Loop Orchestrator in the inspector rather than on the graph. Level 2 · Detail renders only the selected Loop's composite Work Loop Nodes as circular Work-owned artworks plus its `ProjectNodeEdge` connections and explicit terminals. Keep Work → Validation and Validation FAIL / local → Work invariants in the inspector and Run evidence rather than duplicating phases as canvas nodes. Use deterministic layout, fixed-scale panning, smart routed 1.5px paths, explicit Edge labels, unobstructed cycles and the tracked `.ballet/theme.json` theme. Never combine Level 1 Loop nodes or Loop Edges with Level 2 internal nodes or Edges on the same authoring canvas.
...
- **Loop Orchestrator Editor:** Show the Loop Orchestrator once at Loop Engineer Level 1 as a persistent inspector tab beside Selected Loop, never as a Loop or Work Loop Node and never as a detached row below the canvas. On narrow viewports, open the same tab in the inspector Sheet from a visible canvas command. Expose its ExecutionProfile, primary instruction, sorted skills, max repair depth, and max repair attempts. Candidate routing comes from explicit repair Loop Edges; labels and icons must distinguish repair from normal flow without relying on color alone.
- **Loop Engineer Navigation:** Treat the canvas as the dominant workspace surface. Use one compact two-row desktop header: the first row contains one `headline-sm` H1, a single-line active-level explanation or selected-Loop identity, and level-specific actions; the second row contains the compact breadcrumb and Context → Level 1 · Loops → Level 2 · Detail navigation. Keep desktop header controls at 28px and vertical header padding at 8px. URL state owns the active level and selected Loop. Support browser back/forward and canonical deep links. Let Context fill its remaining framed workspace, and let the Level 1 canvas and persistent inspector consume all remaining viewport height while preserving a 34rem canvas minimum; do not reserve a detached Orchestrator row below them. A Level 1 Loop node is keyboard focusable: Space selects it and Enter opens Level 2. On narrow viewports, retain 40px action and level controls, allow their own horizontal scrolling, keep the canvas usable, and move both Selected Loop and Orchestrator inspector tabs into one Sheet without page-level horizontal overflow.
