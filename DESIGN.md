...
- **Loop Engineering** keeps one ordered space canvas visible across Environment, State and Action routes. States run left-to-right by `order`; selecting a State reveals only its Actions top-to-bottom by `priority` beneath that State on the same canvas.
...
Configure workspaces have exactly one sticky toolbar directly below the workspace heading. Status and the current entity are on the leading side; navigation, creation, ordering, projection, save, approval and deletion commands form one trailing action row. At narrow widths the toolbar scrolls internally instead of wrapping commands onto unrelated vertical levels. Action Workspace is the bounded exception: it keeps only a screen-reader heading, has no visible workspace heading or toolbar, and places its atomic save/status row at the end of the form. State selection remains in the canonical sidebar, Action flow opens from the canvas Action node, and Action deletion belongs to the owning State's sortable Action row.
...
- Action Workspace labels only `Validation Agent` and `Work Agent`; generated agent IDs, permissions and TOML descriptions remain canonical but are not rendered or edited there. Each role edits developer instructions, model, reasoning and Skills. Model and reasoning form one selected-model theme card: its accessible borderless exact model-ID button cycles `Sol → Terra → Luna → Sol`, applies the next model's runtime-default reasoning and replaces the former leading planet icon. The compact native discrete slider retains keyboard/touch semantics beneath a small model-artwork thumb and token-colored dot-free rail; exact model ID, slider and explicit reasoning value share one vertically centered row without a visible `Reasoning` caption. Selected Skills use removable shadcn badges with a ghost `+ Skill` chooser. `Save Action` persists Project Config and both Action-agent TOMLs atomically with optimistic hashes from the form-end save row and is disabled during an active Run. Positive readiness prose is omitted; blocking issues remain explicit beside the disabled save. There is no instruction dropdown or machine-local execution editor. Provider, sandbox, network, read-only-root, Computer and pairing controls do not exist.
...
- Loop Engineering and Action flow grids cover the complete internal scroll surface. Pure geometry uses measured surface dimensions and compact deterministic footprints: State nodes form a centered horizontal row, and the selected State's Actions form one vertical column beneath its anchor. Canonical 5 State / 12 Action data fits the 1440×900 canvas region without internal scroll; narrow screens and larger data scroll only inside the labelled canvas. Edges stay centered on the ordered node axes but float clear of visible node bounds; Action IDs sit to the right of their planets and never cross the vertical edge. Selected State uses one semantic border, while keyboard focus remains visible without adding a second border. Visible artwork may shrink without reducing the 44 px route-control hitbox.
