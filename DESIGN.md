...
- **Flow Visualization:** Use a deterministic Loop canvas that renders each composite Work Loop Node as one circular artwork using the Work phase's configured style and `tiny | small | medium | large` size. Keep the Work → Validation and Validation FAIL / local → Work invariants in the inspector and Run evidence rather than duplicating phases as canvas nodes. Render the configured Validation OK Edge to the next Work Loop Node or explicit terminal target. Use fixed-scale panning, smart routed 1.5px paths, explicit Edge labels, unobstructed cycle paths, and at least 208px of horizontal path clearance between node columns. Apply the one tracked `.ballet/theme.json` theme to node labels, active-phase reasoning glow, Edges, and connection points across every Loop canvas. Present the All Loops overview as a dense responsive card grid that may show both composite phases, labels `flow` and `repair` Loop Edges with icons and text, and shows the Loop Orchestrator as a separate routing component.
...
- **Work Loop Node Cards:** Use outside the selected-Loop canvas for Loop definitions, All Loops summaries, inspectors, and detailed Run snapshots. These cards may show Work and Validation as distinct, text-labeled phases and include role type, selected ExecutionProfile name when applicable, status, local attempt, and a concise task, criteria, or Run input section. Do not reuse the composite card as a canvas node.
