...
- **Goals** and **ADRs** are separate Markdown workspaces; Constraints, Use Cases, Instructions and Skills use the same direct editor-only Markdown contract.
...
Configure workspaces have exactly one sticky toolbar directly below the workspace heading. Status and the current entity are on the leading side; navigation, ordering, save, approval and deletion commands form one trailing action row. At narrow widths the entity label may truncate with its full text in a tooltip so the trailing save commands stay visible. If the action row itself exceeds the viewport, it scrolls internally instead of wrapping commands onto unrelated vertical levels. The canvas `+ STATE` and `+ ACTION` nodes switch the right pane to the corresponding creation form. Markdown workspaces (Goals, ADRs, Constraints, Use Cases, Instructions and Skills) combine the workspace name, document ID, status and commands in that single sticky toolbar, with only a screen-reader `h1` and no separate visible heading or introductory prose. Long identity text truncates with its full value in a tooltip; collection counts remain screen-reader-only on narrow screens, while save and blocking states stay in the action row. Action Workspace is the bounded exception: it keeps only a screen-reader heading, has no visible workspace heading or toolbar, and places its atomic save/status row at the end of the form. Action deletion belongs to the owning State's sortable Action row.
