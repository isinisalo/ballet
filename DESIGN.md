...
- **Portable Agent Runtime:** Keep the portable provider, model, reasoning effort, and network intent in the top-level `agents` map of `.ballet/project.json`; keep the runtime backend attachment and absolute read-only roots local. The `Execution` section edits both surfaces as one resolved configuration without hiding which values are portable, and always keeps Runtime, Provider, Model, and Reasoning effort visible when a device is missing or offline. Require an explicit `<device> · <provider>`, model, and reasoning selection before Run; there is no first-runtime fallback. Present workspace-only access as the fixed baseline. An agent definition may be saved without a complete runtime configuration, but Run must show every exact preflight or configuration issue and a direct path to the Runtime Device Registry. Orphan runtime intents and attachments are visible errors, never silently removed. A locally paired computer and its agent attachments persist across restarts and schema migrations; local recovery may reconstruct a missing attachment only from an exact saved provider match and never overwrites an existing selection.
- **Agent Instructions Workbench:** Split the Instructions tab into a responsive live Markdown Preview and Edit workspace. Reuse the project Markdown rendering, technical editor typography, tonal panel boundary, draft validation, word/token metrics, and explicit Save agent action. Keep TOML metadata in the profile rail and do not introduce YAML frontmatter into agent instructions.
