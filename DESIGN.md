...
- **Authoring collections** expose Agents, Skills and ADRs as compact URL-owned nested sidebar lists. Instructions retain their canonical deep link. ADRs and Overview use the shared Markdown editor.
...
Configure workspaces have exactly one sticky toolbar directly below the workspace heading. Status and the current entity are on the leading side; navigation, ordering, save, approval and deletion commands form one trailing action row. At narrow widths the entity label may truncate with its full text in a tooltip so the trailing save commands stay visible. If the action row itself exceeds the viewport, it scrolls internally instead of wrapping commands onto unrelated vertical levels. The canvas `+ STATE` and `+ ACTION` nodes switch the right pane to the corresponding creation form. Markdown workspaces (Overview, ADRs, Instructions and Skills) combine the workspace name, document ID, status and commands in that single sticky toolbar, with only a screen-reader `h1` and no separate visible heading or introductory prose. Long identity text truncates with its full value in a tooltip; collection counts remain screen-reader-only on narrow screens, while save and blocking states stay in the action row. Action Workspace is the bounded exception: it keeps only a screen-reader heading, has no visible workspace heading or toolbar, and places its atomic save/status row at the end of the form. Action deletion belongs to the owning State's sortable Action row.
...
- Markdown-backed project documents use one shared editor-only workbench: sidebar selection, YAML-frontmatter + Markdown body editor and an explicit unsaved-change guard. No rendered Markdown preview or parallel document truth is shown. The workbench starts directly with YAML Frontmatter, followed by Markdown Body, with one content gutter and 12 px vertical spacing below the toolbar. It has no nested panel header, editing-source title or mode banner. Field labels and character counts stay beside their editors; word and token metrics follow the body. Approval hashes, impact warnings and conflict/error recovery remain explicit. In the normal Overview view, the frontmatter input starts within 120 px of the workspace top at 1440×900 and within 160 px including mobile navigation at 390×844.
