...
- **Flow Visualization:** Use a deterministic Loop canvas whose 224×132 composite Work Loop Node visibly separates its Work and Validation phases with text and icons. Show `Work → Validation` and `Validation FAIL / local → Work` as fixed invariants, and render the configured Validation OK Edge to the next Work Loop Node or explicit terminal target. Use fixed-scale panning, smart routed 1.5px paths, explicit Edge labels, and unobstructed cycle paths. Keep at least 208px of horizontal path clearance between full-size node columns. Apply the one tracked `.ballet/theme.json` theme to node labels, ExecutionProfile-derived reasoning glow, Edges, and connection points across every Loop. Phase artwork and size remain definition data selected from the fixed catalog and are not theme-configurable. Present the All Loops overview as a dense responsive card grid that groups composite nodes under each Loop, labels `flow` and `repair` Loop Edges with icons and text, and shows the Loop Orchestrator as a separate routing component.
...
- **Work Loop Node Cards:** Use for Loop definitions and Run snapshots. Show Work and Validation as distinct, text-labeled phases inside one composite card. Include role type, selected ExecutionProfile name when applicable, status, local attempt, and a concise task, criteria, or Run input section.
...
- **Work Loop Node Editor:** Keep Loop identity, required description, State description, initial State JSON, start node, configured Validation OK Edges, terminal targets, and Loop Edges in the Loop definition inspector. Keep composite-node identity, required description, max local attempts, Work, and Validation in the node inspector. Work and Validation each expose their task, role type, appearance, and execution composition only for provider execution. Scheduled is available only to the start Work Node and never to Validation. Show `Work → Validation` and `Validation FAIL / local → Work` as fixed read-only invariants; never present them as free Edge selectors. Profile and primary are required selects, and Skills remain a keyboard-accessible Project-only multi-select whose removable chips are sorted by ID.
- **Loop Orchestrator Editor:** Show the Loop Orchestrator once at All Loops scope as a separate routing component, never as a Work Loop Node. Expose its ExecutionProfile, primary instruction, sorted skills, max repair depth, and max repair attempts. Candidate routing comes from explicit repair Loop Edges; labels and icons must distinguish repair from normal flow without relying on color alone.
- **Run Sheet:** Keep the Run sheet beside the immutable canvas in the established 50/50 workspace. Give the entire sheet one compact WorkLoopNodeRun and active NodeRun metadata header. The left column is an immutable preview of the selected task's System, primary instruction, skills, IDs, origins, and prompt evidence. The right column contains the selected task's durable CLI console and role-specific structured outcome. Human Work returns a WorkOutcome; Human Validation returns `OK` or `FAIL` with the required repair payload. Show finalization commit, changed files, or retained worktree when reported.
- **Loop Canvas Nodes:** Every Work Loop Node is one composite rectangular card with visibly separate Work and Validation rows, role text, and icons. Each phase owns its artwork style and size from the fixed six-style catalog: Classic (`flat`, `luna`, `mars`, `terra`, `sol`) and Planets (`vector-planet`). New phases default to Flat/Medium. Implement artwork as isolated code-native CSS and 24×24 inline-SVG components so selection, ExecutionProfile-derived reasoning glow, Run status, Edge anchors, and reduced-motion behavior remain independent. Human phases show Shield, and Scheduled Work always shows CalendarClock plus one muted Geist schedule line. Keep the exact Work Loop Node ID visible on the card.
