...
- **Loop Theme Editor:** Open the single project theme directly at `/automation/theme`. Use one full-width technical workspace with a fixed-height, non-interactive showcase canvas above dense Node, Edge, and Connection point control zones. The preview shows all nine node artwork styles at representative independently selected sizes, normal/rejected/cross-Loop edges, connection points, and completed/blocked/failed terminal nodes. The editor chrome always uses Ballet design tokens; project-defined hex colors are scoped to Loop canvases only. Preview valid draft changes immediately, keep invalid partial color input visibly adjacent to its field while retaining the last valid preview value, and persist only through an explicit Save command. Do not expose theme identity, copying, assignment, usage counts, or renderer-per-size controls.
...
- **Loop Canvas Nodes:** Every Loop node owns an independent artwork style and size. The artwork catalog is Flat, Luna, Black hole, Satellite, Meteorite, Spaceman, Mars, Terra, and Sol; the size catalog is Tiny 24px, Small 36px, Medium 48px, and Large 64px, and every style supports every size. New executable nodes default to Flat/Medium and terminal nodes to Flat/Tiny. Luna is a gray crater moon; Black hole has a dark core and neutral accretion ring; Satellite uses a metallic body and compact panels; Meteorite is an irregular crater rock; Spaceman is a light astronaut silhouette with a dark visor; Mars uses only `mars-surface`, `mars-highlight`, and `mars-shadow`; Terra and Sol retain their established planetary character. Black hole, Satellite, and Meteorite have no outer node border, while their theme glow and keyboard-focus/selection rings remain visible. Implement artwork as isolated code-native CSS and inline-SVG components so selection, reasoning glow, Run status, edge anchors, and reduced-motion behavior remain independent. CSS gradients are permitted only inside the named Luna, Black hole, Mars, Terra, and Sol artwork presets; Satellite, Meteorite, Spaceman, and Flat use SVG shapes and design tokens. Show an agent avatar only when the global theme enables it; human nodes show Shield, and scheduled nodes always show CalendarClock plus one muted Geist schedule line while still receiving their selected agent's glow and runtime behavior. Keep the exact node ID below every node.
...
- Don't introduce one-off colors, gradients outside the named node artwork presets, ornamental backgrounds, or large decorative illustrations.
