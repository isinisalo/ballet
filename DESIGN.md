...
The current config/domain/snapshot/module baseline is strict v11, while the implemented authoring UI remains Context, Level 1 composition, and selected-Loop Level 2 detail. The accepted ADR-018 target is a coordinated hard cut with exactly **Graph Engineering** and **Loop Engineering**. This design section describes that full target; do not claim its runtime routing or UI is implemented until dispatch, routing, UI, and test evidence pass. During the transition, keep current authoring and direct `followFlow` behavior honest rather than applying target labels to them.
...
- **Local Node Execution:** The implementation keeps portable provider, model, reasoning effort, and network intent in an ExecutionProfile in strict v11 `.ballet/project.json` without a compatibility reader or silent defaults. Keep provider command overrides and checkout-wide absolute `readOnlyRoots` in `.git/ballet/settings.json`. Agent and Scheduled Work Nodes, Agent Validation Nodes, and the Loop Orchestrator select one profile; their editors never edit runtime values. There is no provider fallback, runtime attachment, Agent-specific local policy, or `workspaceAccess` field. A legacy `agentReadOnlyRoots` property is a blocking error with exact remediation and is never silently cleaned up.
