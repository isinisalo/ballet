...
- Action Workspace shows the fixed Validation- and Work-agent names and edits each role's description, developer instructions, model, reasoning and Skill list. `Save Action` persists Project Config and both Action-agent TOMLs atomically with optimistic hashes and is disabled during an active Run. There is no instruction dropdown or machine-local execution editor. Provider, sandbox, network, read-only-root, Computer and pairing controls do not exist.
