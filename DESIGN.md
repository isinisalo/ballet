...
- **Action flow** is a dark industrial workflow: START -> Validation -> done?, with delegate/retry through subordinate Work, evidence returning to Validation, and explicit done/blocked-Feedback terminals plus `1 + maxRetries` text. Its editor has no Action-level Use Case or Agent fields: Use Cases come from the State and each role selects project resources plus machine-local execution settings.
...
- Agent detail follows the compact 15.7 profile composition for Critic/Refinement governance: a narrow Avatar/Execution/Details rail beside Markdown Preview and the source editor. Each Action role exposes provider, capability-derived model/reasoning, network, read-only roots, one primary Instruction and nested Skill multiselect controls. Project-owned composition uses `Save Action`; machine-local binding uses a separate `Save execution`, which affects only future Runs. Computer/pairing controls do not exist.
- Runtimes is a singleton checkout-local diagnostic: daemon status/PID/uptime/last seen/error, active tasks, Codex/Copilot readiness and keyboard-operable Refresh, Restart and Logs. Restart is visibly disabled during active work.
