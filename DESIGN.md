...
- Action execution exposes only Validation- and Work-specific model/reasoning cards. Project-owned composition uses `Save Action`; the machine-local role choices use one atomic `Save execution` and affect only future Runs. Provider, network, read-only-root, Computer and pairing controls do not exist.
