...
- Agent detail follows the compact 15.7 profile composition for Critic/Refinement governance: a narrow Avatar/Execution/Details rail beside Markdown Preview and the source editor. Action execution exposes one shared provider, network switch and read-only roots before the role cards; Validation and Work expose only capability-derived model/reasoning, one primary Instruction and nested Skill multiselect controls. Project-owned composition uses `Save Action`; the entire machine-local Action binding uses one atomic `Save execution`, which affects only future Runs. Computer/pairing controls do not exist.
