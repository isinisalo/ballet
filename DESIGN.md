...
- **Flow Visualization:** Use the compact composite Loop canvas with Step-owned node styles in four fixed sizes: Tiny 24px, Small 36px, Medium 48px, and Large 64px. Use fixed-scale panning, smart routed 1.5px paths, explicit Transition labels, and unobstructed return/cycle paths. Vertically center mixed-size nodes in a lane so their left/right connection points share one horizontal line and direct edges remain straight. Keep at least 208px of horizontal path clearance between full-size Step columns. Apply the one tracked `.ballet/theme.json` theme to node labels, reasoning glow, optional agent avatars, edges, and connection points across every Loop. Node artwork is selected per Step from the fixed catalog and is not theme-configurable. Present the All Loops overview as a dense responsive card grid rather than a second editable graph.
...
- **Loop Canvas Nodes:** Every Step owns one fixed node artwork style, and the style owns its size: Luna, Black hole, Satellite, Meteorite, and Spaceman are Tiny 24px; Mars is Small 36px; Flat and Terra are Medium 48px; Sol is Large 64px. Flat is the neutral technical default for newly created Steps. Luna is a gray crater moon; Black hole has a dark core and neutral accretion ring; Satellite uses a metallic body and compact panels; Meteorite is an irregular crater rock; Spaceman is a light astronaut silhouette with a dark visor; Mars uses only `mars-surface`, `mars-highlight`, and `mars-shadow`; Terra and Sol retain their established planetary character. Implement artwork as isolated code-native CSS and inline-SVG components so selection, reasoning glow, Run status, edge anchors, and reduced-motion behavior remain independent. CSS gradients are permitted only inside the named Luna, Black hole, Mars, Terra, and Sol artwork presets; Satellite, Meteorite, Spaceman, and Flat use SVG shapes and design tokens. Show an agent avatar only when the global theme enables it; human Steps show Shield, and scheduled Steps always show CalendarClock plus one muted Geist schedule line while still receiving their selected agent's glow and runtime behavior. Keep the exact `step.id` below each Step.
- **Terminal Nodes:** Required Approved and Rejected outputs always resolve to a Step, Loop, or terminal target. Default unresolved outputs to `approved → completed` and `rejected → blocked`. Render every completed, blocked, and failed target as its own 24px terminal node and draw its incoming edge. Use Secondary with CheckCircle for completed, Tertiary with Ban for blocked, and Error with XCircle for failed; expose the exact status as both visible label and accessible name. In Configure, terminal nodes remain clickable insertion points that place a new Step between the source branch and its previous terminal target.
- **Loop Canvas Interaction:** Theme normal, rejected, and cross-Loop edges independently as `solid`, `dashed`, or `dotted`; derive muted rejected and return strokes from the theme edge color, and preserve bright 5px connection points, smart-routed paths, cycles, Loop summaries, and the 24px grid. Configure makes Step nodes and Transitions selectable and opens the established 50/50 canvas/sheet workspace. `+ Add loop` first opens a full-width empty canvas with one accessible ghost node and does not persist a draft. Clicking the ghost creates a Flat Medium first Step with completed/blocked defaults, selects the first available agent or Human when no agent exists, and opens the 50/50 editor with compact Loop ID. Closing keeps the unsaved node on canvas; Save requires a valid unique Loop ID and Step. Keep Once/Recurring schedule controls compact and expose scheduler state. Ballet Run keeps the fully resolved global theme, node style, and avatar snapshots immutable, gives the active agent node a restrained Emerald pulse, gives a human-wait node an Amber pulse, and preserves active Transition animation. Pulses and Transition motion become static semantic highlights under `prefers-reduced-motion`.
