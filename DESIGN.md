...
- **Loop Canvas Nodes:** Every Loop node owns an independent artwork style and size. The fixed 27-style catalog contains the existing Classic artwork; six Planets & stations (`black-ice-planet`, `black-planet`, `fire-planet`, `shattered-planet`, `vector-planet`, `battle-station`); six Ships (`ship-arrow`, `ship-fang`, `ship-crescent`, `ship-twin-pod`, `ship-needle`, `ship-hammer`); and six Monsters (`monster-void-eye`, `monster-star-jelly`, `monster-void-manta`, `monster-cosmic-serpent`, `monster-moon-maw`, `monster-astral-kraken`). The size catalog is Tiny 24px, Small 36px, Medium 48px, and Large 64px, and every style supports every size; Ships hide secondary detail at Tiny and expose their compact detail at Small and above. New executable nodes default to Flat/Medium and terminal nodes to Flat/Tiny. Black hole, Satellite, Meteorite, Shattered planet, Ships, and Monsters have no outer node border, while their theme glow and keyboard-focus/selection rings remain visible. Implement artwork as isolated code-native CSS and 24×24 inline-SVG components so selection, reasoning glow, Run status, edge anchors, and reduced-motion behavior remain independent. Artwork uses only Ballet tokens and `color-mix`; supplied stock, watermarked, franchise, and raster images are visual references only and never become application assets. Show an agent avatar only when the global theme enables it; human nodes show Shield, and scheduled nodes always show CalendarClock plus one muted Geist schedule line while still receiving their selected agent's glow and runtime behavior. Keep the exact node ID below every node.
- **Terminal Nodes:** Every v8 Loop owns exactly one `completed`, `blocked`, and `failed` terminal node whose ID equals its type. Transitions target local nodes by ID or another Loop by `{ loop: id }`; default outputs are `approved → completed` and `rejected → blocked`. Render only terminals reachable from the Loop start, using their selected artwork, selected size, and exact ID label without a status icon or status-specific tint. Multiple incoming Transitions share the same terminal node. Terminals have no agent, schedule, outputs, outgoing edge, source handle, or downstream ghost. Their editor locks ID and Type, leaves Agent and both output controls empty and disabled, and permits Description, Node style, and Node size changes. A terminal click selects it for editing and never inserts a Step.
- **Loop Canvas Interaction:** Theme normal, rejected, and cross-Loop edges independently as `solid`, `dashed`, or `dotted`; derive muted rejected and return strokes from the theme edge color, and preserve bright 5px connection points, smart-routed paths, cycles, Loop summaries, and the 24px grid. Each Loop owns one required `summaryStyle`: `route`, `spiral`, `barred-spiral`, `ring`, `edge-on`, `twin-core`, or `irregular-nebula`; Route is the v8 compatibility default. The summary artwork is selected in the always-visible canvas toolbar and reused without geometry changes in 22px linked Loop nodes and All Loops cards. Configure makes executable and terminal nodes plus executable Transitions selectable and opens the established 50/50 canvas/sheet workspace. `+ Add loop` first opens a full-width empty canvas with one accessible ghost node and does not persist a draft. Clicking the ghost creates the Loop's only UI-created executable node as Flat/Medium with completed/blocked defaults, selects the first available agent or Human when no agent exists, and opens the 50/50 editor with compact Loop ID. Do not expose Add/Insert node actions after that first node; existing multi-node Loops remain editable and reorderable. Closing keeps the unsaved node on canvas; Save requires a valid unique Loop ID and executable start node. Persist `.ballet/project.json` as strict v8 with `loops[].nodes`, mandatory `nodeStyle` and `nodeSize`, all three fixed terminal nodes, and a schema-defaulted `summaryStyle`; do not accept v7. Keep Once/Recurring schedule controls compact and expose scheduler state. Ballet Run snapshots every configured Loop summary style in the existing JSON snapshot envelope, propagates that list to child Runs, and keeps fully resolved v8 Loop nodes, terminal nodes, theme, artwork, size, and avatar snapshots immutable. It gives the active agent node a restrained Emerald pulse, gives a human-wait node an Amber pulse, and preserves active Transition animation. Pulses and Transition motion become static semantic highlights under `prefers-reduced-motion`.
