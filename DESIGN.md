...
- Action Workspace labels only `Validation Agent` and `Work Agent`; generated agent IDs, permissions and TOML descriptions remain canonical but are not rendered or edited there. Each role edits developer instructions, model, reasoning and Skills. Model and reasoning form one selected-model theme card: its accessible borderless planet button cycles `Sol → Terra → Luna → Sol`, applies the next model's runtime-default reasoning, and the visible identity is only the exact `gpt-5.6-*` model ID. Its compact native discrete slider retains keyboard/touch semantics beneath a small model-artwork thumb, token-colored dot-free rail and explicit value label without a visible `Reasoning` caption. The card divides identity and slider at workbench width and stacks them narrow. Selected Skills use removable shadcn badges with a ghost `+ Skill` chooser. `Save Action` persists Project Config and both Action-agent TOMLs atomically with optimistic hashes from the form-end save row and is disabled during an active Run. Positive readiness prose is omitted; blocking issues remain explicit beside the disabled save. There is no instruction dropdown or machine-local execution editor. Provider, sandbox, network, read-only-root, Computer and pairing controls do not exist.
...
- Loop Engineering and Action flow grids cover the complete internal scroll surface. Pure geometry uses the measured surface dimensions to distribute ordered nodes across available space while retaining deterministic minimum dimensions and narrow-screen internal scrolling. Every Action edge endpoint and the rendered planet share the same projected center regardless of artwork size; the separate text label does not offset that anchor.
