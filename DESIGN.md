...
- **Reasoning Glow:** Visualize a bound agent's explicit Reasoning effort as seven progressively wider, brighter, and more saturated background-glow levels in this order: `light`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. Smaller efforts are grayer and dimmer; larger efforts approach the resolved Loop theme and node-size accent. In `open-ai`, retain the established pale-gray Luna, gray-green Terra, and gray-orange Sol hues; in `default`, use Primary as the restrained glow hue. `Provider default` and unbound agents do not receive the additional reasoning glow; semantic Run status rings remain independent.
...
- **Portable Agent Runtime:** Keep the portable provider, model, reasoning effort, and network intent in `.ballet/runtime.json`; keep the runtime backend attachment and absolute read-only roots local. The `Execution` section edits both surfaces as one resolved configuration without hiding which values are portable. Require an explicit `<device> · <provider>`, model, and reasoning selection before Run; there is no first-runtime fallback. Present workspace-only access as the fixed baseline. An agent definition may be saved without a complete runtime configuration, but Run must show every exact preflight or configuration issue and a direct path to the Runtime Device Registry. Orphan runtime intents and attachments are visible errors, never silently removed.
...
- **Loop Canvas Nodes:** Every Step owns a `small`, `medium`, or `large` node size, mapped to 28px, 44px, or 64px in every theme. In `open-ai`, map these sizes to the established Luna, Terra, and Sol celestial surfaces without changing their textures, halos, labels, or geometry. In `default`, use a flat `surface-container-highest` circle with a restrained Primary glow and show the bound agent's avatar when present; do not add a decorative gradient. Human and scheduled Steps use the selected size but always show their Shield or CalendarClock semantic mark instead of an agent avatar. Keep the exact `step.id` below the node and add only one muted Geist schedule line for scheduled Steps. CSS radial gradients remain allowed only for the `open-ai` planetary surfaces. Keep related Loop summaries and terminal targets as compact 22px rounded technical markers. Place the exact `step.id` directly below its node, horizontally centered in compact monospace type. Theme normal, rejected, and cross-Loop edges independently as `solid`, `dashed`, or `dotted`; preserve bright 5px connection points, smart-routed return paths, cycles, compact upstream/downstream Loop summaries, and the 24px grid. Configure makes Step nodes and Transitions selectable and opens the established 50/50 canvas/sheet workspace. Keep Once/Recurring schedule controls compact in the Step editor and expose the exact next and latest scheduler states without adding a second canvas surface. Ballet Run keeps theme, size, and avatar snapshots immutable, gives the active agent node a restrained Emerald pulse, gives a human-wait node an Amber pulse, and preserves the active Transition animation. Pulses and Transition motion must become static semantic highlights under `prefers-reduced-motion`. In the All Loops card overview, show each Loop's ID, start Step, agent/human/scheduled Step counts, and directly referenced next Loops.
