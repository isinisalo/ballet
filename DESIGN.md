...
The current implemented baseline is strict v10 with Context, Level 1 composition, and selected-Loop Level 2 detail. The accepted ADR-018 target is a coordinated strict-v11 hard cut with exactly **Graph Engineering** and **Loop Engineering**. This design section describes that target; do not claim it is implemented until domain, schema, snapshot, runtime, API, module, routing, UI, and test evidence all pass. During the transition, keep current v10 behavior honest rather than applying target labels to direct `followFlow` runtime behavior.
...
- **Flow Visualization:** Use exactly two authoring canvases. **Graph Engineering** is the default project-global projection: render one compact rectangular `LoopNode` view node per `ProjectLoop`, one distinct `LoopOrchestrator` control node, and only persisted flow/repair route-policy edges or canonical Run evidence. Arrange larger systems deterministically without shrinking labels into an unreadable row. Label every visible route with icon, kind, and capability-relevant text; do not derive topology from layout, selection, or client-only state. The Orchestrator control node uses Primary/Secondary/Loop Flow/Tertiary semantics in normal states and Error only for blocking or failed state. **Loop Engineering** renders only the selected Loop's composite Work Loop Nodes as circular Work-owned artworks plus internal `ProjectNodeEdge` connections and explicit terminals. Keep Work → Validation and Validation FAIL / local → Work invariants in the inspector and Run evidence rather than duplicating phases as canvas nodes. Use deterministic layout, fixed-scale panning, smart routed 1.5px paths, explicit Edge labels, unobstructed cycles, and the tracked `.ballet/theme.json` theme. Never combine Graph Engineering LoopNodes, Orchestrator control, or project-global routes with Loop Engineering's internal nodes or Edges on the same authoring canvas. Remove Context and numeric level vocabulary in the v11 implementation hard cut.
...
- **Local Node Execution:** The current implementation keeps portable provider, model, reasoning effort, and network intent in an ExecutionProfile in strict v10 `.ballet/project.json`; the accepted target moves the same responsibility to strict v11 without a compatibility reader or silent defaults. Keep provider command overrides and checkout-wide absolute `readOnlyRoots` in `.git/ballet/settings.json`. Agent and Scheduled Work Nodes, Agent Validation Nodes, and the Loop Orchestrator select one profile; their editors never edit runtime values. There is no provider fallback, runtime attachment, Agent-specific local policy, or `workspaceAccess` field. A legacy `agentReadOnlyRoots` property is a blocking error with exact remediation and is never silently cleaned up.
...
- **Work Loop Node Editor:** In Loop Engineering, keep Loop identity, required description, first-class capability metadata, State description, initial State JSON, start node, configured Validation OK Edges, and terminal targets in the Loop definition inspector. Keep project-global graph routes out of Loop Engineering and link back to Graph Engineering for those connections. Keep composite-node identity, required description, max local attempts, Work, and Validation in the node inspector. Work and Validation each expose their task, role type, appearance, and execution composition only for provider execution. Scheduled is available only to the start Work Node and never to Validation. Show `Work → Validation` and `Validation FAIL / local → Work` as fixed read-only invariants; never present them as free Edge selectors. Profile and primary are required selects, and Skills remain a keyboard-accessible Project-only multi-select whose removable chips are sorted by ID.
- **Loop Orchestrator Editor:** Show the Loop Orchestrator exactly once as a distinct control node in Graph Engineering, never as a LoopNode or Work Loop Node and never as detached decoration. Selecting it opens the canonical Orchestrator inspector; on narrow viewports, open that inspector in a Sheet from both the control node and a visible canvas command. Expose its ExecutionProfile, primary instruction, sorted skills, repair limits, and graph route-policy evidence. Flow and repair candidates come only from persisted v11 graph policy and immutable Run evidence; labels and icons must distinguish route kinds without relying on color alone. Normal Orchestrator identity must not use the Error token.
- **Graph and Loop Engineering Navigation:** Treat the canvas as the dominant workspace surface. Use one compact two-row desktop header: the first row contains one `headline-sm` H1 using exactly `Graph Engineering` or `Loop Engineering`, a single-line graph explanation or selected-Loop identity, and view-specific actions; the second row contains a compact breadcrumb and the two-view control. Keep desktop header controls at 28px and vertical header padding at 8px. URL state owns `view=graph | loop` and the selected Loop ID. Support browser back/forward and only the canonical deep links `/automation/loops?view=graph` and `/automation/loops?view=loop&id=<loop-id>`. Graph Engineering is the default. Preserve a 34rem canvas minimum. A Graph LoopNode is keyboard focusable: Space selects it and Enter opens Loop Engineering. On narrow viewports, retain 40px action and view controls, allow their own horizontal scrolling, keep the canvas usable, and move selection/Orchestrator inspectors into one Sheet without page-level horizontal overflow. Do not keep Context, numeric level routes, or disabled future tabs.
- **Run Control Surface:** Lead with one compact canonical-position strip for active Loop, composite Work Loop Node, Work/Validation/Orchestrator role, local attempt, State revision, repair depth, return destination, and finalization. Follow it with a Mission / All Loops visual workspace and one persistent live inspector. Mission keeps the immutable selected-Loop canvas primary, adds a short narration derived only from canonical Run position, and gives its code-native planets restrained status-driven character expressions. All Loops is the focused secondary snapshot: label the Loop Orchestrator separately, render compact Loop planets, keep normal flow routes concise, highlight only the active repair Loop Edge, and show the persisted LIFO return path. The live inspector exposes exact status, node, role, ExecutionProfile, attempt, State revision, repair depth, outcome, and Repair Request values; never invent progress, elapsed time, ETA, dialogue, State, route, or return targets. Below it, use dense timeline, canonical State revision/patch-evidence, and selected ExecutionTask console panels; collapse large JSON values rather than truncating their semantics. Human Work returns a WorkOutcome; Human Validation returns `OK` or `FAIL` with the required repair payload; an Orchestrator awaiting input exposes only resume.
- **Loop Canvas Nodes:** Every Work Loop Node is one circular canvas artwork whose style and 24/36/48/64px size come from the Work phase. Validation appearance remains editable and persisted but does not create a second canvas artwork. Place the exact Work Loop Node ID below the artwork, truncate collision-prone labels visually, and retain the full ID in the interactive accessible name and hover title. Implement artwork as isolated code-native CSS and 24×24 inline-SVG components so selection, active-phase ExecutionProfile reasoning glow, Run status, centered Edge anchors, and reduced-motion behavior remain independent. In Run only, add a small code-native face whose mood maps deterministically from canonical status, mark active Work with a compact Primary Briefcase badge, and mark active Validation with a compact Secondary Shield badge. Character expression is decorative; status remains available as exact text and an accessible name.
- **Terminal Targets:** Current v10 and target v11 terminals are explicit `completed | blocked | failed` internal Edge target values, not authorable nodes. They have no execution composition, schedule, outgoing Edge, or inspector. Show terminal targets only in Loop Engineering as fixed semantic endpoints and in forms as exact target labels.
- **Loop Canvas Interaction:** Theme Validation OK, repair/retry, and project-global flow/repair routes independently as `solid`, `dashed`, or `dotted`; derive muted repair and return strokes from the theme edge color, and preserve bright 5px connection points, smart-routed paths, cycles, Loop summaries, and the 24px grid. The fixed Route summary artwork is neither configurable nor persisted. Loop Engineering makes the single Work-owned artwork and configured Validation OK Edges selectable and opens the existing composite Work/Validation canvas-sheet editor. The current application persists strict v10 until the coordinated cutover; the target persists only strict v11 and must not add a v10 parser, dual-write, or silent defaults. Creating or saving a provider-backed Work or Validation Node requires explicit ExecutionProfile and Project primary-instruction selections; never select a first profile or instruction silently. Ballet Run keeps the fully resolved Root Run snapshot—Loops, composite nodes, internal Edges, graph routes, capabilities, profiles, project resources, System instruction, theme, artwork, and sizes—immutable. It gives the active Node Run a restrained Emerald pulse, gives a human-wait Node Run an Amber pulse, and preserves active route animation. Pulses and route motion become static semantic highlights under `prefers-reduced-motion`.
