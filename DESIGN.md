...
- **Project** contains exactly Overview, Event Storming, User Stories and ADRs in that order.
- **Overview** edits one repository-owned Markdown description containing Purpose, Outcomes, Scope and Shared requirements. The existing editor-only workbench owns the interaction; no parallel config content or section entities exist.
- **Authoring collections** expose Agents and Skills as compact URL-owned nested sidebar lists. Instructions retain their canonical deep link. Overview uses the shared Markdown editor; ADRs use the structured card workspace below.
- **User Stories** use the complete colored story card list and structured card editor over one canonical Markdown file per story. Role / Goal / Benefit, sentence connectors and every numbered GIVEN / WHEN / THEN criterion remain visible. A small text badge shows Draft or Approved. Additional Markdown context and ADR links expand in Details and references; exact approval metadata expands beside the explicit human approval command in the editor.
...
- **Validation-led execution** remains server-owned runtime behavior and is not duplicated as a separate authoring flow. State and Action editors have no User Story fields. Project direction is read only when the selected instruction or Skill explicitly requires the relevant canonical `.ballet/**` documents.
...
Configure workspaces have exactly one sticky toolbar directly below the workspace heading. Status and the current entity are on the leading side; navigation, ordering, save, approval and deletion commands form one trailing action row. At narrow widths the entity label may truncate with its full text in a tooltip so the trailing save commands stay visible. If the action row itself exceeds the viewport, it scrolls internally instead of wrapping commands onto unrelated vertical levels. The canvas `+ STATE` and `+ ACTION` nodes switch the right pane to the corresponding creation form. Markdown workspaces (Overview, Instructions and Skills) combine the workspace name, document ID, status and commands in that single sticky toolbar, with only a screen-reader `h1` and no separate visible heading or introductory prose. Long identity text truncates with its full value in a tooltip; collection counts remain screen-reader-only on narrow screens, while save and blocking states stay in the action row. Action Workspace is the bounded exception: it keeps only a screen-reader heading, has no visible workspace heading or toolbar, and places its atomic save/status row at the end of the form. Action deletion belongs to the owning State's sortable Action row.
...
- Freeform Markdown project documents (Overview, Instructions and Skills) use one shared editor-only workbench: sidebar selection, YAML-frontmatter + Markdown body editor and an explicit unsaved-change guard. No rendered Markdown preview or parallel document truth is shown. The workbench starts directly with YAML Frontmatter, followed by Markdown Body, with one content gutter and 12 px vertical spacing below the toolbar. It has no nested panel header, editing-source title or mode banner. Field labels and character counts stay beside their editors; word and token metrics follow the body. Approval hashes, impact warnings and conflict/error recovery remain explicit. In the normal Overview view, the frontmatter input starts within 120 px of the workspace top at 1440×900 and within 160 px including mobile navigation at 390×844.
...
The card editor saves all three required story parts and zero to fifty complete criteria together. Save returns to the list and focuses the saved card. An explicit delete dialog names the story and explains removal of its criteria. Unsaved edits use the shared navigation guard. File conflicts retain the local draft and expose the latest hash and explicit reload; invalid or missing files remain recoverable without silently overwriting them. Story approval is project evidence with Draft/Approved status; it is not runtime completion. Save cannot approve. Approval requires explicit confirmation of the saved semantic hash, with file hash, approver, time and revision in details. Unsaved, unavailable, conflicted or stale content cannot be approved. Semantic changes return it to Draft; equivalent presentation preserves approval. All persisted content is owned by `.ballet/user-stories/<uuid>.md`.
...
ADR-lista ja editori käyttävät User Stories -näkymän korttirakennetta, Project-otsaketta ja yhtä kiinnittyvää toimintopalkkia. Sivupalkissa ADRs on yksi navigointikohde; lista omistaa tunnistejärjestyksessä esitetyt täysleveät kortit. Jokaisessa kortissa näkyvät ADR-tunniste, Otsikko, Päätös ja Soveltamisala kokonaisina sekä Muokkaa-komento. Sisältöä ei katkaista.

Otsikko käyttää adr-title/on-adr-title-väriparia, Päätös adr-decision/on-adr-decision-paria ja Soveltamisala adr-scope/on-adr-scope-paria. Ne vastaavat User Storyn sinistä, oranssia ja lilaa; väri ilmaisee vain kentän merkityksen. Lista ja lomake näyttävät tekstillä nimetyn väriselitteen. Inter omistaa sisällön, Geist tunnisteet ja kenttälabelit. Kortin padding on 20 px ja korttien väli 16 px. Desktop-sisältögutter on 24 px, mobiilissa 16 px.

Lomakkeen kentät ovat värillisiä, sisällön mukana kasvavia tekstialueita. Tallennettava kenttä on yksi tekstirivi; näkyvä rivittyminen on sallittua ja syötetyt rivinvaihdot korvataan välilyönneillä. Tunniste on kompakti kenttä: luonnissa ehdotetaan suurinta nykyistä numeroa seuraavaa numeroa, tallennetussa recordissa se on vain luettava. Luonti on URL:ssa ?create=adr ja muokkaus ?id=adr-034.

Toimintopalkki näyttää faktuaalisen tallennustilan sekä takaisin-, Peruuta- ja Tallenna ADR -komennot. Tallennus palaa listaan ja kohdistaa tallennettuun korttiin. Tallentamattomia muutoksia suojaa yhteinen navigointivahvistus; stale-hash tai kirjoitusvirhe säilyttää luonnoksen. Virheellistä lähdettä ei ylikirjoiteta. Poistodialogi nimeää tunnisteen ja seuraukset; viittaukset voivat estää poiston. Hyväksyntää, Draft/Approved-statusta, lisätietoja tai hyväksymiskriteereitä ei tallenneta ADR:ään.

390×844-koossa labelit tulevat kenttien yläpuolelle, selite rivittyy ja kontrollit ovat vähintään 44 px. Sivutason vaakaylivuotoa ei sallita. Fokus, virhelabelit, näppäimistöjärjestys ja reduced motion noudattavat yhteistä sopimusta.
