...
- **Loop Canvas Nodes:** Render actions, loop summaries, and terminal output targets as compact 22px rounded rectangles. Keep action identity out of the node; show the source action's exact `action.id` as a monospace label at the end of its forward edge. Keep the Route icon inside loop-summary nodes and render the exact `loop.id` as a horizontal monospace edge label next to the node. Stack compact related-loop summaries vertically with a 48px gap. Loop-summary nodes expose only top and bottom edge anchors; connect loop-to-loop relationships with one direct bottom-to-top edge, while loop-to-action relationships retain the existing cross-loop edge styling and the action node's current anchor rule. Preserve output labels such as `rejected` on rework/return edges and semantic edge colors.
