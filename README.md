...
- `.ballet/project.json` — strict project configuration v8, including each Loop's summary artwork, `loops[].nodes`, independent per-node artwork and size, fixed terminal nodes, schedules, transitions, and agent provider/model/reasoning/network intent;
...
Loop configuration v8 stores executable and terminal nodes together in `loops[].nodes`. Every node selects one of 27 code-native artwork styles and one independent size: Tiny 24 px, Small 36 px, Medium 48 px, or Large 64 px. The catalog contains the nine Classic styles; six Planets & stations (`black-ice-planet`, `black-planet`, `fire-planet`, `shattered-planet`, `vector-planet`, `battle-station`); six Ships (`ship-arrow`, `ship-fang`, `ship-crescent`, `ship-twin-pod`, `ship-needle`, `ship-hammer`); and six Monsters (`monster-void-eye`, `monster-star-jelly`, `monster-void-manta`, `monster-cosmic-serpent`, `monster-moon-maw`, `monster-astral-kraken`). All styles remain valid at every size, while ship detail is optimized for Tiny and Small. Each Loop also selects an independent summary artwork: `route`, `spiral`, `barred-spiral`, `ring`, `edge-on`, `twin-core`, or `irregular-nebula`. Existing v8 files without `summaryStyle` resolve to Route and become explicit on their next normal save. Each Loop has exactly one fixed-ID `completed`, `blocked`, and `failed` terminal. Local transitions use a node ID; cross-Loop transitions retain `{ "loop": "target-loop" }`. Terminal nodes have no agent or outputs, and selecting one edits only its description, artwork, and size. The editor creates only the first executable node for a new Loop and offers no terminal insertion action; existing multi-node Loops remain editable.
