...
| RISK-024 | daemon impersonation/replay, mixed checkout truth, Markdown field loss, unsafe resource path tai duplicate finalization rikkoo execution- tai audit-rajan | checkout-specific 0600 bearer token, loopback/origin gate, checkout/config preflight, fenced lease, draftiin sidottu hash, Gitistä tai snapshotista hash-varmennettu preimage ja TEST-033/TEST-038/TEST-043 | oikean Codex CLI:n pitkä production-like occurrence jää erilliseksi operatiiviseksi evidenssiksi |
...
[Aktiivinen strict cut](../../ARCHITECTURE.md#active-version-matrix) poistaa runtime down-migration -riskin mutta vaatii paikallisen epäyhteensopivan tietokannan/control-plane-staten arkistoinnin tai poiston. Rollback on feature-haaran hylkääminen tai pre-cutover-commitin checkout, ei schema downgrade.
