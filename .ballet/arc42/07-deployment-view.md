...
| DEP-007 | Historical checkout-local strict-v24 installation, superseded by DEP-008 | yksi checkout-kohtainen loopback server/UI, SQLite v22 ja yksi saman checkoutin launchd-daemon; random 0600 bearer-token, polling ja vain Codex CLI, ei pairingia, provider/network/root-valintoja tai remote checkoutia |
| DEP-008 | Checkout-local Action Agents and four Project views | yksi checkout-kohtainen loopback server/UI, fresh SQLite v24, project-local Action Agent TOMLit, kaksi governance TOMLia ja yksi launchd Codex daemon; ei Action binding -taulua tai APIa |
