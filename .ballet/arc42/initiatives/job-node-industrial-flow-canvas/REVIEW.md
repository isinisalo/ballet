...
JNIFC-EVID-001–004 document the original ADR-025 implementation. ADR-027 refinement JNIFC-EVID-005 passed technical, conformance, installed-app and browser review. Human visual acceptance remains a separate project-owner decision, joten initiative säilyy `draft`-tilassa.

## Review-raja

`goal-015`, `adr-023`, `adr-025`, `adr-027`, `QS-020`, `BB-001`, `CON-005` ja `CON-011`; erityisesti no-runtime-change, two-interactive-node, ID-only visibility, same-level junctions/terminals, direct retry routing, responsive layout ja Graph/Graph Node -regressio.
...
- **Fakta JNIFC-EVID-005:** Desktop 1440×900 ja narrow 390×844 näyttävät vain Work/Validation ID:t, Pass?/Retry?-junctionit samalla tasolla, Continue/Escalate-ympyrät samalla tasolla ja vasemmalla leijuvan Retry count -ghostin; retry-edge kiertää ghostin suoraan Workiin.
- **Fakta:** Final validation passed: 49 testitiedostoa / 194 testiä, build, arc42/design-validoinnit, `make latest`, `ballet --no-open` ja terve `ballet status`.
