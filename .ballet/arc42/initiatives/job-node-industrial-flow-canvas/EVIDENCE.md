...
| JNIFC-EVID-005 | QS-020 / REQ-015 | ADR-027 refinement: visible ID-only Work/Validation cards, same-level Pass?/Retry? junctions, same-level Continue/Escalate markers, left-floating Retry count ghost, direct retry edge to Work, no Graph Node Orchestrator or Next job, deterministic wide/narrow layout and unchanged inspector/Sheet behavior. | `adr-027`; `DESIGN.md`; `frontend/src/workspace/automation/JobFlowCanvas.tsx`; `frontend/src/workspace/automation/jobFlowProjection.ts`; `frontend/tests/jobFlowProjection.test.ts`; `frontend/tests/spaceEngineeringCanvas.test.tsx`; `frontend/tests/automationViewJobFlow.test.tsx` | passed | `npm run test` 49 files/194 tests, `npm run lint`, `npm run build`, `npm run validate:arc42`, DESIGN lint, installed `make latest`, `ballet --no-open`, live browser QA 1440×900/390×844, 2026-08-22 | Browser QA proves technical viewport criteria; project-owner visual verdict remains separate. |
