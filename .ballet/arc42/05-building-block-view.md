...
| BB-016 | Project composition and governance Agents | Action-owned instruction/Skill composition, Markdown Critic/Refinement Agents, deterministic Loop Engineering projections and Run Evidence | canonical layers after strict v22/v20 cut |
| BB-017 | Checkout-local daemon execution | server-owned SQLite/worktrees/finalization/evidence, shared-provider/policy Action execution and governance Agent bindings, singleton readiness, polling and fenced CLI execution | `LocalDaemonStore`, `LocalDaemonOrchestrationProvider`, `backend/daemon/**`, `/api/runtimes/local` |
...
- Persistence: SQLite v20 schema/stores ja transaction coordinatorit.
...
- Generic execution infrastructure: checkout-local daemon, Action execution and governance Agent bindings, provider adapters, server-owned worktrees and lease/fencing lifecycle.
