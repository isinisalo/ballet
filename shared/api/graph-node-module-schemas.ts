import { z } from "zod";
import { canvasNodeSizes, canvasNodeStyles, maxJobRetriesLimit, type JsonValue } from "../domain/automation.js";
import {
  maxGraphNodeModuleNodes, maxGraphNodeModuleResourceBodyBytes, maxGraphNodeModuleResources,
  maxGraphNodeModuleRules, maxGraphNodeModuleStringLength, type GraphNodeModulePackageV5
...
const outcomes = z.array(z.object({ outcomeId: localKey, result: z.enum(["PASS", "FAIL"]) }).strict())
  .max(64).refine((values) => unique(values.map(({ outcomeId }) => outcomeId)), "Outcome ids must be unique.");
const target = z.union([
  z.object({ jobNode: localKey }).strict(), z.object({ terminal: z.enum(["PASS", "FAIL"]) }).strict()
]);
...
const routing = z.object({
  ...
  continuation: z.array(z.object({
    key: localKey, sourceJobNode: localKey, result: z.enum(["PASS", "FAIL"]), candidates: z.array(candidate).min(1)
  }).strict()).max(maxGraphNodeModuleRules),
  repair: z.array(z.object({
    key: localKey, sourceJobNode: localKey, capability: nodeCapabilitySchema, candidates: z.array(candidate).min(1)
  }).strict()).max(maxGraphNodeModuleRules)
}).strict();
const orchestrator = z.object({
  ...composition, key: localKey, description: shortText, maxTransitions: z.number().int().min(1).max(256),
  maxRouteAttempts: z.number().int().min(1).max(3), routing
}).strict();
const repairNode = z.object({
  ...composition, key: localKey, description: shortText, task: taskText,
  maxRepairDepth: z.number().int().min(0).max(100), maxRepairAttempts: z.number().int().min(1).max(100)
}).strict();
...
export const graphNodeModulePackageV5Schema = z.object({
  format: z.literal("ballet-graph-node-module"), version: z.literal(5),
  manifest: z.object({
  ...
  permissions: z.object({ network, externalWrites: z.union([z.literal(false), z.literal("requires-human-authorization")]) }).strict(),
  ...
  capabilities: z.object({
    ...
    recommendedGraphRoutes: z.array(z.object({
      direction: z.enum(["incoming", "outgoing"]), outcomeId: localKey, result: z.enum(["PASS", "FAIL"]),
  ...
  graphNode: z.object({
    ...
    strategy: z.object({ kind: z.literal("agent_v1"), orchestrator }).strict(),
  ...
  const localOrchestrator = pkg.graphNode.strategy.orchestrator;
  const ruleIds = [localOrchestrator.routing.start.key,
    ...localOrchestrator.routing.continuation.map((rule) => rule.key),
    ...localOrchestrator.routing.repair.map((rule) => rule.key)];
  if (!unique(ruleIds)) context.addIssue({ code: "custom", path: ["graphNode", "strategy", "orchestrator", "routing"], message: "Routing rule keys must be unique." });
  const compositions = [localOrchestrator, ...(pkg.graphNode.repairNode ? [pkg.graphNode.repairNode] : []),
  ...
  const candidates = [localOrchestrator.routing.start, ...localOrchestrator.routing.continuation,
    ...localOrchestrator.routing.repair].flatMap((rule) => rule.candidates);
  for (const jobKey of jobKeys) if (!candidates.some((value) => "jobNode" in value.target && value.target.jobNode === jobKey)) {
    context.addIssue({ code: "custom", path: ["graphNode", "strategy", "orchestrator", "routing"], message: `Job Node ${jobKey} is unreachable.` });
  }
  for (const terminal of ["PASS", "FAIL"] as const) if (!candidates.some((value) => "terminal" in value.target && value.target.terminal === terminal)) {
    context.addIssue({ code: "custom", path: ["graphNode", "strategy", "orchestrator", "routing"], message: `Terminal ${terminal} is unreachable.` });
  }
  localOrchestrator.routing.continuation.forEach((rule, index) => {
    if (!jobKeys.has(rule.sourceJobNode)) context.addIssue({ code: "custom", path: ["graphNode", "strategy", "orchestrator", "routing", "continuation", index], message: "Unknown source Job Node." });
  });
  localOrchestrator.routing.repair.forEach((rule, index) => {
    if (!pkg.graphNode.repairNode) context.addIssue({ code: "custom", path: ["graphNode", "strategy", "orchestrator", "routing", "repair", index], message: "Repair routing requires a Repair Node." });
    if (!jobKeys.has(rule.sourceJobNode)) context.addIssue({ code: "custom", path: ["graphNode", "strategy", "orchestrator", "routing", "repair", index], message: "Unknown source Job Node." });
  });
}) as z.ZodType<GraphNodeModulePackageV5>;
