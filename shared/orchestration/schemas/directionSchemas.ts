...
export const useCaseAuthoringSchema = z.object({
  ...
  examples: z.array(useCaseExampleSchema).min(1).max(CONTRACT_LIMITS.examplesPerUseCase),
  successGoals: z.array(nonEmptyTextSchema).min(1).max(CONTRACT_LIMITS.referencesPerItem),
  failureGoals: z.array(nonEmptyTextSchema).min(1).max(CONTRACT_LIMITS.referencesPerItem),
  expectedOutcomes: z.array(nonEmptyTextSchema).min(1).max(CONTRACT_LIMITS.referencesPerItem),
...
export const directionSchema = z.object({
  goals: z.array(directionReferenceSchema).max(CONTRACT_LIMITS.directionItems),
  adrs: z.array(directionReferenceSchema).max(CONTRACT_LIMITS.directionItems),
  constraints: z.array(constraintSchema).max(CONTRACT_LIMITS.directionItems),
  useCases: z.array(useCaseSchema).max(CONTRACT_LIMITS.useCases)
}).strict();
