...
import { ROLE_OUTCOME_VERSION } from "../versions.js";
...
const precheckDecisionSchema = z.discriminatedUnion("decision", [
  ...
  z.object({
    ...
    correctiveActions: z.array(nonEmptyTextSchema).min(1).max(CONTRACT_LIMITS.evidenceItems), evidence
  }).strict()
]);
...
const criticProposalSchema = z.object({
  ...
  recommendedCorrectiveActions: z.array(nonEmptyTextSchema).min(1).max(CONTRACT_LIMITS.evidenceItems),
...
const refinementFileProposalSchema = z.object({
  ...
  proposedContent: z.string().max(CONTRACT_LIMITS.instruction).optional(),
  rationale: nonEmptyTextSchema,
...
export const refinementOutcomeSchema = z.object({
  ...
  files: z.array(refinementFileProposalSchema).min(1).max(CONTRACT_LIMITS.proposalFiles),
  ...
  risks: z.array(nonEmptyTextSchema).max(CONTRACT_LIMITS.evidenceItems),
  validationPlan: z.array(z.enum(["instruction_contract", "resource_contract", "relevant_tests"]))
    .min(1).max(CONTRACT_LIMITS.evidenceItems),
