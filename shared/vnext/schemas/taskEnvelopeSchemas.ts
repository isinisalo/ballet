...
const base = {
  version: z.literal(VNEXT_TASK_ENVELOPE_VERSION),
  taskId: idSchema,
  environmentRunId: idSchema,
  snapshotSha256: sha256Schema,
  instruction: z.string().trim().min(1).max(VNEXT_LIMITS.instruction),
  context: z.json()
};
const actionBase = {
  ...base,
...
export const validationPrecheckTaskEnvelopeSchema = z.object({
  ...
  workAttempts: z.number().int().nonnegative().max(1 + VNEXT_LIMITS.maxRetries),
  maxRetries: z.number().int().nonnegative().max(VNEXT_LIMITS.maxRetries)
}).strict();
...
export const workTaskEnvelopeSchema = z.object({
  ...
  workAttempt: z.number().int().positive().max(1 + VNEXT_LIMITS.maxRetries),
...
export const validationPostworkTaskEnvelopeSchema = z.object({
  ...
  workAttempt: z.number().int().positive().max(1 + VNEXT_LIMITS.maxRetries),
  retriesRemaining: z.number().int().nonnegative().max(VNEXT_LIMITS.maxRetries),
...
export const refinementTaskEnvelopeSchema = z.object({
  ...
  allowedPaths: z.array(nonEmptyTextSchema).max(VNEXT_LIMITS.proposalFiles),
