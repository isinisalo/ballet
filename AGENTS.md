...
- Graph Engineering käyttää capability-first GraphNode-kortteja ja yhtä Graph-tason Reward Decision Model -työtilaa. Graph Node näyttää vain valitun GraphNoden ordered Action Node -kortit. Local Decision Model-, Orchestrator-, Repair- tai runtime-tulosnäkymiä ei lisätä authoring-projektioksi.
...
- Action-flow on authoring-projektio, ei runtime-ohjain. Continue/Escalate eivät ole painikkeita eikä canvas tallenna next-targetia. Runtime suorittaa Action Nodet array-järjestyksessä, käyttää Work→Validationia ja bounded `retry | escalate` -semantiikkaa; Graph Reward-MDP valitsee vain GraphNode-optionin.
...
- Graph Node Module package-, katalogi-, install-, export- ja provenance-primitiveet ovat geneerisiä platform-ominaisuuksia. Moduulien nimet, capabilityt, instructionit, skillsit ja external-write-metadata ovat `.ballet/graph-node-library/**`-dataa; runtime lukee vain materialisoitua project-local dataa. Peer-GraphNode-targetit, probabilityt ja rewardit kuuluvat project-global Reward-MDP:hen, eivät pakettiin.
...
- Repositoryn oletusgraphissa ovat project-local GraphNodet DESIGN, PLAN, BUILD, DEPLOY ja VERIFY sekä DONE-terminal. Yksi Graph-tason Reward-MDP valitsee immutable compiled policysta GraphNode-optionin; GraphNode suorittaa Action Nodet array-järjestyksessä. Work→Validation ja bounded `retry | escalate` ovat Action Noden kiinteitä invariantteja.
