...
import type { ExecutionPolicy, WorkspaceAccess } from "../../../shared/domain/runtime.js";
...
export class WorkspacePermissionPolicy implements RuntimePermissionPolicy {
  ...
  constructor(workspace: string, private readonly policy: ExecutionPolicy, private readonly workspaceAccess: WorkspaceAccess) {
    ...
    switch (request.kind) {
      ...
      case "command":
        return this.workspaceAccess === "workspace-write"
          && Boolean(request.path && within(request.path, this.workspace) && commandIsWorkspaceScoped(request.command ?? "", this.policy.network));
      case "mcp":
      case "unknown":
        return false;
    }
