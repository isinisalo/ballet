...
export class LeaseAwareJobRunner {
  ...
  // eslint-disable-next-line complexity -- Lease, cancellation and finalization form one fail-closed lifecycle boundary.
  ...
  private async executeAdapter(
    ...
    for await (const event of adapter.execute({
      ...
      outputSchema: schema,
      signal,
      permissionPolicy: this.options.permissionPolicy ?? new WorkspacePermissionPolicy(workspace.path, spec.runtime.policy, spec.workspaceAccess)
    })) {
  ...
  private async verifyRuntimePolicy(adapter: CliRuntimeAdapter, spec: ExecutionSpec, signal: AbortSignal): Promise<void> {
    ...
    if (spec.workspaceAccess === "workspace-write" && !probe.policyCapabilities.workspaceWrite) {
