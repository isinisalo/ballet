...
const spec = (): ExecutionSpecV18 => ({
  version: 18, taskId: "task-1", kind: "agent_execution", environmentRunId: "run-1", agentRunId: "agent-run-1",
  evidence: {
    compositionVersion: 16, role: "validation", phase: "precheck",
    subject: { kind: "action_agent", actionId: "action-1", role: "validation", agent: actionAgent },
  ...
  runtime: { subject: { kind: "action_agent", actionId: "action-1", role: "validation", agentId: actionAgent.id }, provider: "codex", cliVersion: "999.0.0", model: "gpt",
...
const actionAgent = { id: "ballet-action-validation-action-1", name: "ballet-action-validation-action-1",
  ...
  reasoningEffort: "high", contentSha256: "d".repeat(64) };
