...
export class CopilotSdkAdapter implements CliRuntimeAdapter {
  ...
  private async run(request: RuntimeExecutionRequest, queue: AsyncEventQueue<RuntimeEvent>): Promise<void> {
    ...
    try {
      ...
      queue.push({ type: "session.started", sessionId: session.sessionId });
      ...
      if (request.outputSchema && structured.error) throw new Error(`Copilot output failed schema validation: ${structured.error}`);
      if (!output.trim()) throw new Error("Copilot completed without a final response.");
      queue.push({ type: "execution.completed", sessionId: session.sessionId, output, structuredOutput: structured.value });
      queue.close();
    } finally {
...
const enforceCopilotSandbox = async (
  session: CopilotSessionLike,
  request: Pick<RuntimeExecutionRequest, "workingDirectory" | "workspaceAccess" | "policy">
): Promise<void> => {
  ...
  await session.rpc.options.update({
    ...
    sandboxConfig: {
      ...
      userPolicy: {
        filesystem: {
          readwritePaths: request.workspaceAccess === "workspace-write" ? [workingDirectory] : [],
          readonlyPaths: [
