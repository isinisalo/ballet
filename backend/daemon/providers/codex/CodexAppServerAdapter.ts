...
const CLIENT_INFO = { name: "ballet-daemon", title: "Ballet local runtime", version: "0.1.0" };
...
export class CodexAppServerAdapter implements CliRuntimeAdapter {
  ...
  private async run(request: RuntimeExecutionRequest, queue: AsyncEventQueue<RuntimeEvent>): Promise<void> {
    ...
    const state: CodexTurnState = { sessionId: undefined, finalText: "" } as CodexTurnState;
    ...
    try {
      ...
      await this.initialize(client);
      state.threadId = await this.openThread(client, request, queue);
      active.threadId = state.threadId;
      queue.push({ type: "session.started", sessionId: state.threadId });
      ...
      queue.push({ type: "execution.completed", sessionId: state.threadId, output: state.finalText, structuredOutput });
      queue.close();
    } finally {
  ...
  private async openThread(
    ...
    const common = {
      ...
      approvalPolicy: "never",
      sandbox: request.workspaceAccess
    };
    if (request.sessionId) {
      try {
        const resumed = await client.request("thread/resume", { ...common, threadId: request.sessionId });
        return threadIdFromCodexResult(resumed) ?? request.sessionId;
      } catch (error) {
        queue.push({ type: "diagnostic", level: "warning", message: "Codex session could not be resumed; starting a new thread.", data: String(error) });
      }
    }
