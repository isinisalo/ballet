...
export class BalletDaemon {
  ...
  private async checkoutHeartbeat(inspectionId?: string): Promise<DaemonHeartbeatPayload["checkout"]> {
    ...
    const inspected = inspectionId
      ? await this.options.git.refreshManagedProject(projectId, this.controller.signal)
      : await this.options.git.inspectManagedProject(projectId, this.controller.signal);
    ...
    return {
      ...
      configHash: inspected.configHash,
