...
export class GitWorkspaceManager {
  ...
  async inspectManagedProject(projectId: string, signal?: AbortSignal): Promise<GitCheckoutStatus & { configHash: string; snapshotHash: string }> {
    const repository = this.repositoryPath(projectId);
    const [status, snapshot, projectConfig] = await Promise.all([
      inspectGitCheckout(repository, signal),
      this.snapshots.capture(repository),
      readFile(path.join(repository, ".ballet", "project.json"), "utf8")
    ]);
    const parsedConfig = projectConfigurationV21Schema.parse(JSON.parse(projectConfig) as unknown);
    return { ...status, configHash: sha256(canonicalJson(parsedConfig)), snapshotHash: snapshot.hash };
  }

  async refreshManagedProject(projectId: string, signal?: AbortSignal): Promise<GitCheckoutStatus & { configHash: string; snapshotHash: string }> {
    const repository = this.repositoryPath(projectId);
    const before = await inspectGitCheckout(repository, signal);
    if (before.codeDirty) throw new Error("Managed project checkout is dirty and cannot be refreshed safely.");
    await runGit(["fetch", "--prune", "origin"], { cwd: repository, signal });
    await runGit(["merge", "--ff-only", "@{upstream}"], { cwd: repository, signal });
    return this.inspectManagedProject(projectId, signal);
  }
