...
export class ControlPlaneDatabase {
  ...
  private createSchema(db: Database.Database): void {
    ...
    if (existingVersion && existingVersion.value !== SCHEMA_VERSION) {
      throw new Error(`Unsupported control-plane schema version ${existingVersion.value}. Expected ${SCHEMA_VERSION}. No migration is available.`);
    }
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS project_checkouts (
        ...
        snapshot_hash TEXT, dirty INTEGER NOT NULL DEFAULT 0, last_inspected_at TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
