/* eslint-disable max-lines -- This facade is the single transaction/event boundary over focused control-plane stores. */
...
export class ControlPlaneService {
  ...
  async startAgentRun(agentId: string, input?: string): Promise<AgentRun> {
    ...
    const spec: ExecutionSpec = {
      ...
      agentRunId: runId, input, workspaceAccess: "read-only", agent, runtime: check.runtime, project: check.project, createdAt: timestamp
    };
  ...
  enqueueLoopStep(input: {
    ...
    input?: string; workspaceAccess: ExecutionSpec["workspaceAccess"]; agentSnapshot: ExecutionAgentSnapshot;
  }): ExecutionTask {
    ...
    const spec: ExecutionSpec = {
      ...
      workspaceAccess: input.workspaceAccess, runtime: check.runtime, project: check.project, createdAt: this.options.now().toISOString()
    };
  ...
  rootFinalizationReport(rootRunId: string): RootFinalizationReport | undefined {
    return this.options.finalizations.reported(rootRunId);
  }
