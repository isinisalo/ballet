...
export const createUiRouter = (options: ControlPlaneRouterOptions): express.Router => {
  ...
  const trusted: express.RequestHandler = (_req, _res, next) => next();
  const read = options.trustedLocalUi ? trusted : adminAuth(options.service, false);
  const write = options.trustedLocalUi ? trusted : adminAuth(options.service, true);
  ...
  router.get("/agents/:agentId/execution", read, (req, res) => res.json(options.service.getBinding(parseParams(agentExecutionParamsSchema, req).agentId) ?? null));
  router.put("/agents/:agentId/execution", write, (req, res) => {
