  ...
  updateCheckout(deviceId: string, input: {
    ...
    this.connection().prepare(`
      INSERT INTO project_checkouts (
        checkout_id, project_id, device_id, repository_url, checkout_path, head_sha, config_hash, snapshot_hash,
      ...
      ON CONFLICT(project_id, device_id) DO UPDATE SET checkout_id = excluded.checkout_id,
        ...
        head_sha = excluded.head_sha, config_hash = excluded.config_hash, snapshot_hash = excluded.snapshot_hash, dirty = excluded.dirty,
    ...
    `).run(identityChanged || !existing ? uuid() : existing.checkout_id, device.project_id, deviceId,
      input.repositoryUrl, input.path, input.headSha ?? null, input.configHash ?? null, input.snapshotHash ?? null,
      input.dirty ? 1 : 0, input.lastInspectedAt ?? timestamp,
      identityChanged || !existing ? timestamp : existing.created_at, timestamp);
  }
