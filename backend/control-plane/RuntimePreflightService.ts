...
export class RuntimePreflightService {
  ...
  private projectSnapshot(project: RegisteredProject, deviceId: string, agentId: string, issues: RuntimePreflightIssue[]): ExecutionProjectSnapshot | undefined {
    ...
    if (!checkout || !checkout.headSha || !checkout.configHash || !checkout.snapshotHash) {
      issues.push({ agentId, code: "offline", message: "Runtime checkout has not reported an immutable head, Project Config hash and resource snapshot hash." });
      return undefined;
    }
    ...
    return { ...base, snapshotHash: checkout.snapshotHash };
  }
}
