...
export class RootFinalizationStore {
  ...
  reported(rootRunId: string): RootFinalizationReport | undefined {
    const row = this.get(rootRunId);
    return row?.status === "reported" && row.report_json
      ? JSON.parse(row.report_json) as RootFinalizationReport
      : undefined;
  }
