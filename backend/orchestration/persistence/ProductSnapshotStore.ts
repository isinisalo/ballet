...
export class ProductSnapshotStore {
  ...
  create(input: ProductSnapshotSeed): void {
    ...
    if (run.status !== "completed") throw new ConflictError("Product Snapshot requires a completed Environment Run.");
    if (run.branch !== input.branch || run.worktree_path !== input.worktreePath || run.base_commit !== input.baseCommit) {
      throw new ConflictError("Product Snapshot worktree identity differs from its Environment Run.");
    }
  ...
  requireByRun(environmentRunId: string): Record<string, unknown> {
    ...
    if (!row) throw new NotFoundError(`Environment Run ${environmentRunId} has no Product Snapshot.`);
