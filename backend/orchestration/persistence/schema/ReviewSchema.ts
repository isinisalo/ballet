export const reviewSchema = `
  ...
  CREATE TABLE feedback_entries (
    ...
    refinement_proposal_id TEXT REFERENCES refinement_proposals(refinement_proposal_id) ON DELETE SET NULL,
    continuation_run_id TEXT REFERENCES environment_runs(environment_run_id) ON DELETE SET NULL,
    approval_json TEXT,
  ...
  CREATE TABLE feedback_status_events (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    feedback_entry_id TEXT NOT NULL REFERENCES feedback_entries(feedback_entry_id) ON DELETE CASCADE,
    from_status TEXT NOT NULL CHECK (from_status IN ('open','in_refinement')),
    to_status TEXT NOT NULL CHECK (to_status IN ('open','in_refinement','resolved','dismissed')),
    actor_type TEXT NOT NULL CHECK (actor_type IN ('human','refinement','continuation')),
    actor_id TEXT NOT NULL,
    refinement_proposal_id TEXT,
  ...
  CREATE TABLE refinement_proposals (
    ...
    expected_behavioral_improvement TEXT NOT NULL,
  ...
  CREATE TABLE refinement_proposal_decisions (
    ...
    acknowledged_local_commit_and_continuation INTEGER NOT NULL CHECK (
      acknowledged_local_commit_and_continuation IN (0,1)
      AND (decision = 'rejected' OR acknowledged_local_commit_and_continuation = 1)
    ),
    decided_by TEXT NOT NULL,
    decided_at TEXT NOT NULL,
    rationale TEXT
