...
import type { FeedbackSeed } from "../../../shared/orchestration/persistence.js";
...
export class FeedbackStore {
  ...
  require(feedbackEntryId: string): Record<string, unknown> {
    const row = this.connection().prepare("SELECT * FROM feedback_entries WHERE feedback_entry_id = ?")
      .get(feedbackEntryId);
    if (!row) throw new NotFoundError(`Feedback ${feedbackEntryId} was not found.`);
  ...
  transition(input: {
    ...
    this.connection().transaction(() => {
      ...
      if (updated.changes !== 1) throw new ConflictError(`Feedback ${input.feedbackEntryId} status changed or is terminal.`);
      ...
      if (!actorId) throw new ConflictError("Feedback transition requires trusted provenance.");
      this.connection().prepare(`
