...
export class EnvironmentWorkspaceManager implements OrchestrationWorkspacePort, ProductFinalizationPort, GovernanceWorkspaceBoundary {
  ...
  async prepare(runId: string, expectedBaseCommit?: string): Promise<{ worktreePath: string; branch: string }> {
    ...
    if (status) throw new Error("Commit or stash project changes before starting an immutable orchestration Run.");
    const branch = `ballet/orchestration-run/${safeId(runId)}`;
    ...
    if (expectedBaseCommit && base !== expectedBaseCommit) {
      throw new Error("Project HEAD changed after immutable orchestration planning.");
    }
