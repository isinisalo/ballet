...
export const createOrchestrationRouter = ({ controller, actor }: OrchestrationRouterOptions): express.Router => {
  ...
  router.get("/project", route(async (_req, res) => res.json(controller.authoring.project())));
...
const registerDocumentRoutes = (
  ...
  for (const [collection, kind] of [["instructions", "instruction"], ["skills", "skill"]] as const) {
    ...
    router.post(`/${collection}`, route(async (req, res) => {
      ...
      res.status(201).json(controller.authoring.createResource(kind, input.id, input.content, input.expectedHash));
    }));
    router.get(`/${collection}/:id`, route(async (req, res) =>
      res.json(controller.authoring.document(kind, parseParams(idParamsSchema, req).id))));
    router.put(`/${collection}/:id`, route(async (req, res) => {
      const input = parseBody(putResourceSchema, req); const { id } = parseParams(idParamsSchema, req);
      res.json(controller.authoring.updateResource(kind, id, input.content, input.expectedHash));
    }));
    router.delete(`/${collection}/:id`, route(async (req, res) => {
...
const registerEnvironmentRoutes = (router: express.Router, controller: ApiController): void => {
  router.get("/environment", route(async (_req, res) => res.json(controller.authoring.environment())));
  router.put("/environment", route(async (req, res) => {
    const input = parseBody(putEnvironmentSchema, req); res.json(controller.authoring.putEnvironment(input.environment, input.expectedConfigHash));
  }));
  router.post("/environment/states/reorder", route(async (req, res) => {
    const input = parseBody(reorderSchema, req); res.json(controller.authoring.reorderStates(input.orderedIds, input.expectedConfigHash));
  }));
  router.post("/environment/states", route(async (req, res) => {
    const input = parseBody(putStateSchema, req); res.status(201).json(controller.authoring.createState(input.state, input.expectedConfigHash));
  }));
  router.get("/environment/states/:stateId", route(async (req, res) =>
    res.json(controller.authoring.state(parseParams(stateParamsSchema, req).stateId))));
  ...
  router.post("/environment/states/:stateId/actions/reprioritize", route(async (req, res) => {
    ...
    res.json(controller.authoring.reprioritizeActions(stateId, input.orderedIds, input.expectedConfigHash));
  }));
  router.post("/environment/states/:stateId/actions", route(async (req, res) => {
    ...
    res.status(201).json(controller.authoring.createAction(stateId, input.action, input.expectedConfigHash));
  }));
  router.get("/environment/states/:stateId/actions/:actionId", route(async (req, res) => {
    const { stateId, actionId } = parseParams(actionParamsSchema, req); res.json(controller.authoring.action(stateId, actionId));
  }));
  router.put("/environment/states/:stateId/actions/:actionId", route(async (req, res) => {
    ...
    res.json(controller.authoring.updateAction(stateId, input.action, {
...
const registerRunRoutes = (
  ...
  router.get("/environment-runs", route(async (_req, res) => res.json(controller.runQueries.listRuns())));
  router.get("/environment-runs/:runId", route(async (req, res) => res.json(controller.runQueries.run(parseParams(runParamsSchema, req).runId))));
  ...
  router.get("/environment-runs/:runId/evidence", route(async (req, res) =>
    res.json(controller.runQueries.evidence(parseParams(runParamsSchema, req).runId))));
  router.get("/environment-runs/:runId/events", (req, res, next) => {
    try {
      ...
      const facts = controller.runQueries.eventFacts(runId, after);
...
const registerReviewRoutes = (
  ...
  router.get("/critic/schedules", route(async (_req, res) => res.json(controller.reviewQueries.listCritic("schedules"))));
  router.get("/critic/runs", route(async (_req, res) => res.json(controller.reviewQueries.listCritic("runs"))));
  router.get("/critic/proposals", route(async (_req, res) => res.json(controller.reviewQueries.criticProposals())));
  router.get("/critic/proposals/:id", route(async (req, res) => res.json(controller.reviewQueries.criticProposal(parseParams(idParamsSchema, req).id))));
  ...
  router.get("/refinement/runs", route(async (_req, res) => res.json(controller.reviewQueries.listRefinement("runs"))));
  router.get("/refinement/proposals", route(async (_req, res) => res.json(controller.reviewQueries.listRefinement("proposals"))));
  router.get("/refinement/proposals/:id", route(async (req, res) => res.json(await controller.reviewQueries.refinementProposal(parseParams(idParamsSchema, req).id))));
  ...
  router.get("/refinement/proposals/:id/apply", route(async (req, res) =>
    res.json(controller.reviewQueries.refinementApplyStatus(parseParams(idParamsSchema, req).id))));
  router.get("/refinement/proposals/:id/continuation", route(async (req, res) =>
    res.json(controller.reviewQueries.continuation(parseParams(idParamsSchema, req).id))));
};
