...
import { nodeOutcomeJsonSchema, nodeOutcomeSchema } from "../../shared/api/runtime-schemas.js";
import type {
...
export const toExecutionTask = (row: ExecutionTaskRow): ExecutionTask => {
  ...
  return {
    ...
    outcome: row.outcome_json ? parseOutcome(row.outcome_json, row.task_id) : undefined,
...
export const assertExecutionSpecEvidence = (spec: ExecutionSpec): void => {
  ...
  if (spec.evidence.outputSchemaSha256 !== sha256(schemaJson)
    || schemaJson !== canonicalJson(nodeOutcomeJsonSchema)) {
...
const parseOutcome = (source: string, taskId: string): CanonicalNodeOutcome => {
  const parsed = nodeOutcomeSchema.safeParse(JSON.parse(source));
  if (!parsed.success) throw new Error(`Execution task ${taskId} has an invalid persisted outcome.`);
  return parsed.data;
};
