...
import { selectRelevantHistory, serializeTaskEnvelopeV5, TaskEnvelopeValidationError } from "./TaskEnvelopeV5.js";

const jobEnvelope = (): JobTaskEnvelopeV5 => ({
  version: 5,
  role: "job",
...
describe("Task Envelope V5", () => {
  it("serializes deterministically with canonical State and history order", () => {
    const first = serializeTaskEnvelopeV5(jobEnvelope());
    const reordered = jobEnvelope();
    reordered.state.value = { alpha: 1, beta: 2 };
    reordered.relevantHistory.reverse();
    const second = serializeTaskEnvelopeV5(reordered);
    expect(first.serialized).toBe(second.serialized);
    expect(first.sha256).toBe(second.sha256);
    expect(first.envelope).toMatchObject({ version: 5, role: "job", jobNode: { id: "produce" }, jobAttempt: 2 });
  ...
  it("measures Unicode as UTF-8 bytes while preserving exact content", () => {
    ...
    const result = serializeTaskEnvelopeV5(value);
  ...
  it("includes Validation Job evidence and a generic Orchestrator allowlist", () => {
    ...
    expect(serializeTaskEnvelopeV5(validation).envelope).toMatchObject({ role: "validation", validationNode: { id: "produce-validation" }, jobOutcome: { role: "job" } });
    expect(serializeTaskEnvelopeV5(orchestrator).envelope).toMatchObject({ role: "orchestrator", allowedCandidates: [{ id: "a-loop" }, { id: "z-loop" }], orchestrationRequest: { id: "request" } });
  });
});

describe("Task Envelope V5 limits", () => {
  it.each([
    ["State hash mismatch", (value: JobTaskEnvelopeV5) => { value.state.sha256 = "0".repeat(64); }, /State SHA-256/],
    ["unknown field", (value: JobTaskEnvelopeV5) => Object.assign(value, { hiddenReasoning: "private" }), /unrecognized/i],
    ["oversized State", (value: JobTaskEnvelopeV5) => { value.state.value = "x".repeat(262_144); value.state.sha256 = jsonSha256(value.state.value); }, /maximum is 262144 bytes/],
    ["oversized relevant history", (value: JobTaskEnvelopeV5) => { value.relevantHistory = Array.from({ length: 8 }, (_, sequence) => ({ sequence, nodeRunId: `node-${sequence}`, role: "job", state: "completed", summary: "x".repeat(9_000), stateRevision: sequence })); }, /relevant history.*maximum/i],
    ["oversized resume context", (value: JobTaskEnvelopeV5) => { value.resume = { question: "q".repeat(16_000), context: "c".repeat(16_000), response: "r".repeat(16_000) }; }, /resume context.*maximum/i]
  ])("fails closed for %s", (_label, mutate, message) => {
    const value = jobEnvelope();
    mutate(value);
    expect(() => serializeTaskEnvelopeV5(value)).toThrow(message);
  });
  ...
  it("rejects duplicate Orchestrator candidates and an oversized request", () => {
    ...
    expect(() => serializeTaskEnvelopeV5(value)).toThrow(/duplicate allowed candidate/);
    value.allowedCandidates = [value.allowedCandidates[0]!];
    value.orchestrationRequest.completionEvidence = "x".repeat(65_536);
    expect(() => serializeTaskEnvelopeV5(value)).toThrow(TaskEnvelopeValidationError);
    expect(() => serializeTaskEnvelopeV5(value)).toThrow(/Orchestration Request.*maximum/i);
  });
  ...
  it("fails the complete Envelope limit without truncating valid fields", () => {
    ...
    expect(() => serializeTaskEnvelopeV5(value)).toThrow(/Task Envelope is .*maximum is 393216 bytes/);
    expect(value.state.value).toHaveLength(250_000);
  });
});

const orchestratorEnvelope = (): OrchestratorTaskEnvelopeV5 => ({
  version: 5,
  role: "orchestrator",
