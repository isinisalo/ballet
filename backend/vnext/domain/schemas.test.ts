...
import {
  VNEXT_EXECUTION_SPEC_VERSION, VNEXT_PROJECT_CONFIG_VERSION, VNEXT_ROLE_OUTCOME_VERSION,
  VNEXT_TASK_ENVELOPE_VERSION, agentRunSchema, approveUseCase, projectConfigurationV20Schema,
...
const outcomeBase = { version: VNEXT_ROLE_OUTCOME_VERSION, summary: "Summary", checks };

describe("strict vNext outcome schemas", () => {
  it.each([
...
describe("task and runtime boundary schemas", () => {
  const base = {
    version: VNEXT_TASK_ENVELOPE_VERSION,
...
describe("Project Configuration v20 boundary", () => {
  it("reserves strict next versions without changing active v19", () => {
    expect(defaultProjectConfiguration().version).toBe(19);
    expect(VNEXT_PROJECT_CONFIG_VERSION).toBe(20);
    expect(VNEXT_TASK_ENVELOPE_VERSION).toBe(10);
    expect(VNEXT_ROLE_OUTCOME_VERSION).toBe(10);
    expect(VNEXT_EXECUTION_SPEC_VERSION).toBe(12);
  });
