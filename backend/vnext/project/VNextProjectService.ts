...
import type { Constraint, DirectionReference, UseCase } from "../../../shared/vnext/direction.js";
import { approveUseCase, invalidateUseCaseApproval } from "../../../shared/vnext/direction.js";
...
import type { VNextProjectDefinition, VNextProjectDefinitionPort } from "../runtime/EnvironmentRunPlanner.js";
import { VNextMarkdownRepository } from "./VNextMarkdownRepository.js";
import { VNextProjectRepository } from "./VNextProjectRepository.js";
import { VNextReferenceIndex, type VNextDocumentKind } from "./VNextReferenceIndex.js";

const runFile = promisify(execFile);
export type DirectionValue = DirectionReference | Constraint | UseCase;

export class VNextProjectService implements VNextProjectDefinitionPort {
  constructor(
    readonly root: string,
    readonly projects: VNextProjectRepository,
    readonly documents: VNextMarkdownRepository
  ) {}

  putDirection(input: {
    kind: Exclude<VNextDocumentKind, "instruction" | "skill">;
    id: string;
    ...
    if (input.id !== input.value.id) throw new VNextConflictError("Direction path id differs from document value id.");
    const loaded = this.projects.load();
    if (loaded.configHash !== input.expectedConfigHash) throw new VNextConflictError("vNext Project Config optimistic hash is stale.");
  ...
  removeDirection(input: {
    ...
    const loaded = this.projects.load();
    if (loaded.configHash !== input.expectedConfigHash) throw new VNextConflictError("vNext Project Config optimistic hash is stale.");
    const references = new VNextReferenceIndex(loaded.config).for(input.kind, input.id);
    const blockers = [
    ...
    if (blockers.length > 0) throw new VNextConflictError(`${input.kind} ${input.id} has reference blockers: ${blockers.join(", ")}.`);
  ...
  approveUseCase(id: string, expectedConfigHash: string, actor: TrustedHumanActor, at: string): string {
    ...
    if (!useCase) throw new VNextNotFoundError(`Use Case ${id} was not found.`);
    if (useCase.status === "approved") throw new VNextConflictError(`Use Case ${id} is already approved.`);
  ...
  revokeUseCase(id: string, expectedConfigHash: string): string {
    ...
    const useCase = loaded.config.direction.useCases.find((candidate) => candidate.id === id);
    if (!useCase) throw new VNextNotFoundError(`Use Case ${id} was not found.`);
    if (useCase.status !== "approved") throw new VNextConflictError(`Use Case ${id} is not approved.`);
  ...
  async load(): Promise<VNextProjectDefinition> {
    const loaded = this.projects.load();
    const resources = (["instruction", "skill"] as const).flatMap((kind) =>
      this.documents.list(kind).map(({ id, content }) => ({
        kind, id, content, relativePath: path.relative(this.root, path.join(
          this.documents.dataRoot, kind === "instruction" ? "instructions" : "skills", `${id}.md`
        ))
      }))
    );
...
const replaceDirectionValue = (
  ...
  if (kind === "goal") direction.goals = replace(direction.goals, input as DirectionReference);
    ...
    if ((!existing && next.status !== "draft") || (existing && next.status !== existing.status)) {
      throw new VNextConflictError("Use Case approval status changes require the dedicated human command.");
    }
    if ((!existing || existing.status === "draft") && next.approval) {
      throw new VNextConflictError("Draft Use Case cannot carry approval metadata.");
    }
    if (existing?.status === "approved" && JSON.stringify(next.approval) !== JSON.stringify(existing.approval)) {
      throw new VNextConflictError("Use Case approval metadata is owned by the dedicated human command.");
    }
