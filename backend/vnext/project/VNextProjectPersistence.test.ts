...
import { validVNextProjectConfig } from "../testing/VNextProjectFixtures.js";
...
import { VNextProjectService } from "./VNextProjectService.js";
import { VNextReferenceIndex } from "./VNextReferenceIndex.js";

const cleanups: Array<() => void> = [];
afterEach(() => cleanups.splice(0).forEach((cleanup) => cleanup()));

describe("isolated vNext project persistence", () => {
  test("uses only the explicit v20 path and never falls back to v19", () => {
    const context = fixture(false);
    writeFileSync(path.join(context.root, ".ballet", "project.json"), JSON.stringify({ version: 19 }));
    expect(() => context.projects.load()).toThrow("vNext Project Config");
    expect(context.projects.loadOptional()).toBeUndefined();
  });
  ...
  test("rejects config and document symlinks without following them", () => {
    const context = fixture(false);
    const target = path.join(context.root, "target.json"); writeFileSync(target, JSON.stringify(validVNextProjectConfig()));
  ...
  test("indexes deletion blockers across Direction, Environment, instructions, and skills", () => {
    const index = new VNextReferenceIndex(validVNextProjectConfig());
...
const fixture = (initialize = true) => {
  const root = mkdtempSync(path.join(tmpdir(), "ballet-vnext-project-"));
  mkdirSync(path.join(root, ".ballet"), { recursive: true });
  const manager = new VNextConnection(path.join(root, "state.sqlite"));
  const database = () => manager.connection(); database();
  const projects = new VNextProjectRepository(path.join(root, ".ballet", "vnext", "project.json"), database);
  const documents = new VNextMarkdownRepository(path.join(root, ".ballet", "vnext"), database);
  if (initialize) projects.save(validVNextProjectConfig(), "absent");
  const service = new VNextProjectService(root, projects, documents);
