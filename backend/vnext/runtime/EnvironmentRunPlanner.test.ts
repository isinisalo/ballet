...
import { approveUseCase, type UseCase } from "../../../shared/vnext/direction.js";
...
import type { RuntimeCapabilitySnapshot } from "../../../shared/vnext/runtime.js";
import { EnvironmentRunPlanner, type VNextProjectDefinition } from "./EnvironmentRunPlanner.js";
import { parseVNextStructuredOutput } from "./StructuredOutputValidator.js";
...
describe("strict vNext provider output", () => {
  test("rejects markdown wrappers, unknown fields, invalid enum and phase mismatch", () => {
    ...
    expect(parseVNextStructuredOutput(`\`\`\`json\n${valid}\n\`\`\``, { role: "validation", phase: "precheck" }).success).toBe(false);
    expect(parseVNextStructuredOutput(valid.replace('"checks":[]', '"checks":[],"extra":true'), { role: "validation", phase: "precheck" }).success).toBe(false);
    expect(parseVNextStructuredOutput(valid.replace('"decision":"done"', '"decision":"continue"'), { role: "validation", phase: "precheck" }).success).toBe(false);
    expect(parseVNextStructuredOutput(valid, { role: "validation", phase: "postwork" }).success).toBe(false);
  });
});

const definition = (): VNextProjectDefinition => {
  const draft: UseCase = {
