...
import type { CreateEnvironmentRunInput, FeedbackSeed, ProductSnapshotSeed } from "../../../shared/vnext/persistence.js";
import type { ValidationOutcome, WorkOutcome } from "../../../shared/vnext/outcomes.js";
...
import { VNextExecutionStore, type StoredExecutionTask } from "../persistence/VNextExecutionStore.js";
...
import type { VNextRuntimeProvider } from "./RuntimeProvider.js";
...
export class EnvironmentRuntimeService {
  ...
  constructor(
    ...
    private readonly provider: VNextRuntimeProvider,
    ...
    this.execution = new VNextExecutionStore(this.connection);
  ...
  async processNext(): Promise<boolean> {
    ...
    const parsed = parseVNextStructuredOutput(terminal.raw, { role: agent.role, phase: agent.phase });
    if (!parsed.success) {
      this.execution.finishTask(taskId, terminal.providerOutcomeKey, "failed", {
