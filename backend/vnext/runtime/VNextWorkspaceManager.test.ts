...
import { VNextWorkspaceManager } from "./VNextWorkspaceManager.js";

const roots: string[] = [];
afterEach(() => roots.splice(0).forEach((root) => rmSync(root, { recursive: true, force: true })));

describe("vNext governance workspace lifetime", () => {
  test("materializes the immutable Product commit read-only and cleans it after capture", async () => {
    ...
    const manager = new VNextWorkspaceManager(root, worktrees, (kind) => kind);
