...
} from "../../../shared/vnext/httpContracts.js";

export interface VNextRouterOptions {
  controller: VNextApiController;
  actor(): TrustedHumanActor;
}

export const createVNextRouter = ({ controller, actor }: VNextRouterOptions): express.Router => {
  ...
  router.use((_req, res) => res.status(404).json({ error: "Unknown vNext API route." }));
  return router;
};

const registerDocumentRoutes = (
  router: express.Router, controller: VNextApiController, actor: () => TrustedHumanActor
): void => {
...
const registerEnvironmentRoutes = (router: express.Router, controller: VNextApiController): void => {
...
const registerRunRoutes = (router: express.Router, controller: VNextApiController): void => {
...
export const vNextDocumentCollections: Readonly<Record<string, VNextDocumentKind>> = Object.freeze({
  goals: "goal", adrs: "adr", constraints: "constraint", "use-cases": "use-case",
