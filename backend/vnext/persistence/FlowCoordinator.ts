...
export class FlowCoordinator {
  ...
  advance(environmentRunId: string, expectedRevision: number, at: string): StoredActionExecution {
    return this.connection().transaction(() => {
      ...
      if (!["pending", "running"].includes(run.status) || run.activeActionExecutionId) {
        throw new VNextConflictError(`Environment Run ${environmentRunId} cannot advance from ${run.status}.`);
      }
      const state = this.firstOpenState(environmentRunId);
      if (state.status === "blocked" || state.status === "cancelled" || state.status === "interrupted") {
        throw new VNextConflictError(`State ${state.stateExecutionId} gates later work.`);
      }
      ...
      if (action.status !== "pending") throw new VNextConflictError(`Action ${action.actionExecutionId} is already active or terminal.`);
      ...
      if (selected.changes !== 1) throw new VNextStaleStateError(`Action ${action.actionExecutionId} changed during selection.`);
      ...
      if (advanced.changes !== 1) throw new VNextStaleStateError(`Environment Run ${environmentRunId} changed during advance.`);
  ...
  completeState(stateExecutionId: string, expectedRevision: number, at: string): StoredStateExecution {
    return this.connection().transaction(() => {
      ...
      if (state.revision !== expectedRevision) throw new VNextStaleStateError(`State ${stateExecutionId} revision is stale.`);
      if (state.status !== "running") throw new VNextConflictError(`State ${stateExecutionId} is not running.`);
      const actions = this.runs.actions(stateExecutionId);
      if (actions.length === 0 || actions.some(({ status }) => status !== "done")) {
        throw new VNextConflictError(`State ${stateExecutionId} cannot complete before every Action is done.`);
      }
      ...
      if (updated.changes !== 1) throw new VNextStaleStateError(`State ${stateExecutionId} changed during completion.`);
  ...
  completeEnvironment(input: ProductSnapshotSeed, expectedRevision: number): StoredEnvironmentRun {
    return this.connection().transaction(() => {
      ...
      if (run.status !== "running" || run.activeStateExecutionId || run.activeActionExecutionId || run.activeAgentRunId) {
        throw new VNextConflictError(`Environment Run ${run.environmentRunId} is not ready for completion.`);
      }
      const states = this.runs.states(run.environmentRunId);
      if (states.length === 0 || states.some(({ status }) => status !== "done")) {
        throw new VNextConflictError("Environment cannot complete before every State is done.");
      }
      const updated = this.connection().prepare(`
      ...
      if (updated.changes !== 1) throw new VNextStaleStateError(`Environment Run ${run.environmentRunId} changed during completion.`);
  ...
  stop(environmentRunId: string, expectedRevision: number, kind: "cancelled" | "interrupted", at: string): StoredEnvironmentRun {
    return this.connection().transaction(() => {
      const run = this.requireRunRevision(environmentRunId, expectedRevision);
      if (!["pending", "running"].includes(run.status)) throw new VNextConflictError(`Environment Run ${environmentRunId} is terminal.`);
  ...
  private requireRunRevision(environmentRunId: string, expectedRevision: number): StoredEnvironmentRun {
    ...
    if (run.revision !== expectedRevision) throw new VNextStaleStateError(`Environment Run ${environmentRunId} revision is stale.`);
    return run;
  }
  ...
  private firstOpenState(environmentRunId: string): StoredStateExecution {
    ...
    if (!state) throw new VNextConflictError(`Environment Run ${environmentRunId} has no unfinished State.`);
    return state;
  }
  ...
  private firstOpenAction(stateExecutionId: string): StoredActionExecution {
    ...
    if (!action) throw new VNextConflictError(`State ${stateExecutionId} has no unfinished Action.`);
    return action;
  }
  ...
  private activateState(state: StoredStateExecution, at: string): void {
    ...
    if (updated.changes !== 1) throw new VNextStaleStateError(`State ${state.stateExecutionId} changed during activation.`);
  }
}
