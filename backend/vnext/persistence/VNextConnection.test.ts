...
import { VNEXT_DATABASE_SCHEMA_VERSION, VNextTableNames } from "./VNextSchema.js";
...
describe("strict SQLite v16 open behavior", () => {
  it("creates the exact v16 inventory in an empty database", () => {
    ...
    expect(names).toEqual([...VNextTableNames].sort());
    expect(database.connection.prepare("SELECT value FROM metadata WHERE key = 'schema_version'").get())
      .toEqual({ value: String(VNEXT_DATABASE_SCHEMA_VERSION) });
  ...
  it("reopens v16 without recreating persisted data", () => {
    ...
    const reopened = new VNextConnection(database.databasePath);
  ...
  it.each(["15", "999"])("fails closed for schema %s and leaves a sentinel unchanged", (version) => {
    const { databasePath } = legacyDatabase(version);
  ...
  it("fails closed for an unversioned unknown database", () => {
    ...
    raw.close();
    expect(() => new VNextConnection(databasePath).connection()).toThrow(/schema unknown.*left unchanged/i);
  });
});
...
const tempDirectory = (): string => {
  const directory = mkdtempSync(path.join(tmpdir(), "ballet-vnext-version-"));
  directories.push(directory);
  return directory;
};

const legacyDatabase = (version: string): { databasePath: string } => {
  const directory = tempDirectory();
