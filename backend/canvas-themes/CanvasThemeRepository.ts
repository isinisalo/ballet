...
import {
  defaultCanvasTheme,
  type CanvasTheme,
  type CanvasThemeIssue
} from "../../shared/domain/canvasTheme.js";
import { CanvasThemeConflictError } from "./CanvasThemeErrors.js";
import { parseCanvasTheme } from "./canvasThemeValidation.js";

export interface CanvasThemeLoadResult {
  theme: CanvasTheme;
  issues: CanvasThemeIssue[];
}

const themeRelativePath = ".ballet/theme.json";

export class CanvasThemeRepository {
  async load(root: string): Promise<CanvasThemeLoadResult> {
    const fallback = structuredClone(defaultCanvasTheme);
  ...
  async update(root: string, value: unknown): Promise<CanvasTheme> {
    const theme = parseCanvasTheme(value);
    const directory = await ensureBalletDirectory(root);
    const filename = themeFilename(root);
    const fileStatus = await status(filename);
    if (fileStatus && (!fileStatus.isFile() || fileStatus.isSymbolicLink())) {
      throw new CanvasThemeConflictError("Theme is not backed by an ordinary JSON file.");
    }
...
const ensureBalletDirectory = async (root: string): Promise<string> => {
  ...
  if (!directoryStatus?.isDirectory() || directoryStatus.isSymbolicLink()) {
    throw new CanvasThemeConflictError(".ballet must be an ordinary directory.");
  }
  return directory;
};
