...
import { validateActionInstruction } from "@shared/vnext/instructionContract";
...
export function ResourceWorkspace({ kind, resources, references, locked, onSave }: { kind: "instructions" | "skills"; resources: ResourceDocument[]; references: ReferenceEntry[]; locked: boolean; onSave(id: string, content: string, expectedHash: string | "absent", creating: boolean): Promise<void> }) {
  const [selectedId, setSelectedId] = useState<string>(); const [creating, setCreating] = useState(false);
