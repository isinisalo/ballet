...
import type { ActionDefinition, CriticConfiguration, ExecutionProfile, StateDefinition } from "@shared/vnext/environment";
...
type Mutation = ReturnType<typeof useVNextMutation>;
type DirectionKind = "goals" | "adrs" | "constraints" | "use-cases";

export function VNextConfigureOutlet({ route, data, navigate, mutation }: { route: RouteState; data: VNextConfigureData; navigate(path: string): void; mutation: Mutation }) {
  ...
  const saveDirection = async (kind: DirectionKind, value: DirectionReference | Constraint | UseCase, markdown: string, creating: boolean) => { const document = data[directionDataKey(kind)].find((item) => item.id === value.id); await mutation.run(() => vNextApi.saveDirection(kind, value, markdown, configHash, document?.contentHash ?? "absent", creating)); };
  const updateProject = (next: typeof config) => mutation.run(() => vNextApi.putProject(next, configHash)).then(() => undefined);
  let content;
  switch (route.vNextView) {
    case "direction": content = <DirectionWorkspace direction={config.direction} documents={{ goals: data.goals, adrs: data.adrs, constraints: data.constraints }} references={data.references.entries} locked={locked} onSave={(kind, value, markdown, creating) => saveDirection(kind, value, markdown, creating)} onDelete={async (kind, value) => { const document = data[directionDataKey(kind)].find((item) => item.id === value.id); if (document) await mutation.run(() => vNextApi.deleteDirection(kind, value.id, configHash, document.contentHash)); }} />; break;
    case "use-cases": content = <UseCasesWorkspace useCases={config.direction.useCases} documents={data.useCases} locked={locked} environmentUsage={config.environment.states.flatMap((state) => [...state.useCaseIds, ...state.actions.flatMap((action) => action.useCaseIds)])} onSave={(value, markdown, creating) => saveDirection("use-cases", value, markdown, creating)} onApprove={async (value) => { await mutation.run(() => vNextApi.approveUseCase(value.id, configHash)); }} onDraft={async (value) => { await mutation.run(() => vNextApi.returnUseCaseToDraft(value.id, configHash)); }} />; break;
    case "environment": content = <EnvironmentWorkspace environment={config.environment} issues={validateRunnableEnvironment(config.environment, config.direction)} locked={locked} navigate={navigate} onMove={async (id, delta) => { const orderedIds = reorderStates(config.environment.states, id, delta).map((state) => state.id); await mutation.run(() => vNextApi.reorderStates(orderedIds, configHash)); }} />; break;
    case "state": { const state = config.environment.states.find((item) => item.id === route.stateId); content = <StateWorkspace state={state} locked={locked} navigate={navigate} onSave={async (next: StateDefinition) => { await mutation.run(() => vNextApi.updateState(next, configHash)); }} onMoveAction={async (id, delta) => { if (!state) return; const orderedIds = reprioritizeActions(state.actions, id, delta).map((action) => action.id); await mutation.run(() => vNextApi.reprioritizeActions(state.id, orderedIds, configHash)); }} />; break; }
    case "action": { const state = config.environment.states.find((item) => item.id === route.stateId); const action = state?.actions.find((item) => item.id === route.actionId); content = <ActionWorkspace stateId={state?.id} action={action} profiles={config.executionProfiles} instructions={data.instructions} skills={data.skills} locked={locked} navigate={navigate} onSave={async (next: ActionDefinition) => { if (state) await mutation.run(() => vNextApi.updateAction(state.id, next, configHash)); }} />; break; }
    case "instructions": content = <ResourceWorkspace kind="instructions" resources={data.instructions} references={data.references.entries} locked={locked} onSave={async (id, source, hash, creating) => { await mutation.run(() => vNextApi.saveResource("instructions", id, source, hash, creating)); }} />; break;
    case "skills": content = <ResourceWorkspace kind="skills" resources={data.skills} references={data.references.entries} locked={locked} onSave={async (id, source, hash, creating) => { await mutation.run(() => vNextApi.saveResource("skills", id, source, hash, creating)); }} />; break;
    ...
    case "critic": content = <CriticWorkspace critic={config.critic} statuses={data.schedules} locked={locked} onSave={(critic: CriticConfiguration) => updateProject({ ...config, critic })} onManual={async () => { await mutation.run(vNextApi.manualCritic); }} />; break;
