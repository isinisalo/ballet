  ...
  return (
    <>
      ...
      <Collapsible open={advancedOpen} onOpenChange={onAdvancedOpenChange}>
        ...
        <CollapsibleContent className="grid gap-3 border-t border-divider-strong pt-3"><Field className="gap-1.5"><FieldLabel htmlFor={`read-only-roots-${agentId}`}>Additional read-only roots</FieldLabel><Textarea id={`read-only-roots-${agentId}`} className="min-h-20 font-mono text-xs" disabled={!supportsRoots} placeholder="One absolute path per line" value={editor.form.policy.readOnlyRoots.join("\n")} onBlur={() => onPersist(editor.saveIfValid())} onChange={(event) => setRoots(event.target.value)} /></Field></CollapsibleContent>
      </Collapsible>
    </>
  );
}
