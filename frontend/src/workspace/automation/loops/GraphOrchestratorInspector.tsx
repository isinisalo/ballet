...
function OrchestratorPolicyEvidence({ projection }: { projection: GraphEngineeringProjection }) {
  ...
  return <section aria-label="Orchestrator route policy and live Run evidence" className="grid gap-2 border-b border-divider-strong pb-4">
    ...
    <div className="grid grid-cols-3 gap-1.5 text-center font-mono text-[0.62rem]"><span className="rounded border border-divider-strong p-1.5">{projection.edges.filter(({ kind }) => kind === "flow").length}<small className="block text-muted-foreground">flow</small></span><span className="rounded border border-tertiary/40 p-1.5 text-tertiary">{projection.edges.filter(({ kind }) => kind === "repair").length}<small className="block text-muted-foreground">repair</small></span><span className="rounded border border-secondary/40 p-1.5 text-secondary">{active.length}<small className="block text-muted-foreground">active</small></span></div>
