...
export function LoopHandlerAgentInstructions({ step, agents, snapshot }: {
  ...
  const agent = step.type === "human" ? undefined : agents.find((candidate) => candidate.id === step.agentId);
  const Icon = step.type === "human" ? ShieldCheck : step.type === "scheduled" ? CalendarClock : agent ? Bot : TriangleAlert;
        ...
        <header className="mb-2 flex items-start justify-between gap-3 border-b border-divider-strong pb-2">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              {step.type !== "human" ? <p className="truncate font-mono text-[0.65rem] text-muted-foreground">{step.agentId}</p> : null}
            </div>
          </div>
