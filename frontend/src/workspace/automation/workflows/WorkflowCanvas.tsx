...
import { workflowEdgeDomAttributes, workflowEdgeStyle } from "./workflowEdgeStyle";
...
export function toWorkflowReactFlowEdges(
  ...
  return layoutEdges.map((workflowEdge) => {
    const sourceNode = nodeByKey.get(workflowEdge.sourceNodeKey);
    const targetNode = nodeByKey.get(workflowEdge.targetNodeKey);
    const isAnimated = workflowEdge.key === animatedEdgeId;
    ...
    return {
      ...
      animated: isAnimated,
      className: isAnimated ? "workflow-edge-animated" : undefined,
      style: workflowEdgeStyle(workflowEdge, targetNode, isAnimated),
      interactionWidth: 16,
      selectable: false,
      focusable: false,
      reconnectable: false,
      domAttributes: workflowEdgeDomAttributes(workflowEdge, isAnimated)
    };
  });
}
