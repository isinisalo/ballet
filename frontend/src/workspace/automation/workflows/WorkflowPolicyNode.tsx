...
export function WorkflowPolicyNode({
  ...
  const workflowId = record.workflowId ?? context.selectedWorkflowId;
  const editable = workflowId === context.selectedWorkflowId;
  ...
  return (
    <div
      data-workflow-id={folded || !editable ? undefined : workflowId}
      data-workflow-step-index={folded || !editable ? undefined : record.index}
      onPointerDown={folded || !editable ? undefined : (event) => context.onStepPointerDown(event, workflowId, record.index)}
