...
import { ActionNodeCreateDialog, ActionNodeRenameDialog, GraphNodeCreateDialog, GraphNodeRenameDialog } from "./GraphNodeCrudDialogs";
...
export function AutomationView({ data, level, section, graphNodeId, actionNodeId, saveAutomation, navigate, setNavigationBlocker }: {
  ...
  const actionNode = graphNode?.jobNodes.find((node) => node.id === actionNodeId);
  ...
  useEffect(() => {
    if (!parse.success || activeStrategy?.kind !== "ssp_v2" || level === "action_node") { setPolicyResult(undefined); setPolicyLoading(false); return; }
  ...
  return <EngineeringShell level={level} section={section} graphNodeId={graphNode?.id} graphNodeTitle={graphNode?.description} actionNodeId={actionNode?.id} actionNodeTitle={actionNode?.description} actions={actions} navigate={navigate}>
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      /> : <CapabilityCards nodes={draft.graph.graphNodes} kind="Graph Node" locked={locked} onAdd={() => setDialog({ kind: "create" })} onOpen={(id) => navigate(automationGraphNodePath(id))} onEdit={(id) => setDialog({ kind: "edit", id })} onRename={(id) => setDialog({ kind: "rename", id })} onDelete={(id) => setDialog({ kind: "delete", id })} />
        ...
        /> : <CapabilityCards nodes={graphNode.jobNodes} kind="Action Node" locked={locked} onAdd={() => setDialog({ kind: "create" })} onOpen={(id) => navigate(automationActionNodePath(graphNode.id, id))} onEdit={(id) => setDialog({ kind: "edit", id })} onRename={(id) => setDialog({ kind: "rename", id })} onDelete={(id) => setDialog({ kind: "delete", id })} deleteIssues={(id) => jobNodeReferences(graphNode, id)} />
          : level === "action_node" && graphNode && actionNode ? <div className="flex min-h-0 flex-1"><JobFlowCanvas job={actionNode} selected={selection === "work" || selection === "validation" ? selection : undefined} locked={locked} onWork={() => setSelection("work")} onValidation={() => setSelection("validation")} /><EngineeringInspector model={automationInspectorModel(selection, graphNode, actionNode, locked)} profiles={data.executionProfiles} instructions={data.instructions} onChange={(field, value) => setDraft((config) => updateAutomationSelection(config, selection, graphNodeId, actionNodeId, field, value))} onClose={() => setSelection("none")} /></div>
...
function SectionTabs({ level, section, graphNodeId, navigate }: { level: EngineeringLevel; section?: EngineeringSection; graphNodeId?: string; navigate: WorkspaceNavigation["navigate"] }) {
  const entries = level === "graph" ? [["Capability Graph", "capabilities"], ["Decision Model", "decision-model"]] as const : [["Actions", "actions"], ["Local Decision Model & Repair", "local-decision-model"]] as const;
  return <nav className="flex shrink-0 gap-1 border-b border-divider-strong bg-card px-3 pt-2" aria-label="Engineering sections">{entries.map(([label, value]) => <Button key={value} size="sm" variant={section === value || (!section && value === entries[0][1]) ? "secondary" : "ghost"} className="rounded-b-none" onClick={() => navigate(level === "graph" ? automationGraphPath(value as "capabilities" | "decision-model") : automationGraphNodePath(graphNodeId!, value as "actions" | "local-decision-model"))}>{label}</Button>)}</nav>;
}
...
function CrudDialogs({ dialog, setDialog, data, draft, setDraft, graphNode, level, navigate }: { dialog?: { kind: "create" | "rename" | "edit" | "delete"; id?: string }; setDialog: (value: { kind: "create" | "rename" | "edit" | "delete"; id?: string } | undefined) => void; data: AppData; draft: ProjectAutomationConfig; setDraft: React.Dispatch<React.SetStateAction<ProjectAutomationConfig>>; graphNode?: ProjectAutomationConfig["graph"]["graphNodes"][number]; level: EngineeringLevel; navigate: WorkspaceNavigation["navigate"] }) {
  ...
  const actionEntity = graphNode?.jobNodes.find(({ id }) => id === dialog?.id);
  const entity = level === "graph" ? graphEntity : actionEntity;
  ...
  return <>
    {level === "graph" ? <GraphNodeCreateDialog {...createProps} onCreate={(input) => setDraft((config) => addGraphNode(config, createGenericGraphNode(input)))} /> : graphNode ? <ActionNodeCreateDialog {...createProps} onCreate={(input) => setDraft((config) => addJobNode(config, graphNode.id, createGenericJobNode(input)))} /> : null}
    {dialog?.kind === "rename" && entity ? level === "graph" ? <GraphNodeRenameDialog open onOpenChange={(open) => { if (!open) setDialog(undefined); }} currentId={entity.id} existingIds={draft.graph.graphNodes.map(({ id }) => id)} onRename={(id) => { setDraft((config) => renameGraphNode(config, entity.id, id)); navigate(automationGraphNodePath(id), { bypassBlocker: true }); }} /> : graphNode ? <ActionNodeRenameDialog open onOpenChange={(open) => { if (!open) setDialog(undefined); }} currentId={entity.id} existingIds={graphNode.jobNodes.map(({ id }) => id)} onRename={(id) => { setDraft((config) => renameJobNode(config, graphNode.id, entity.id, id)); navigate(automationGraphNodePath(graphNode.id), { bypassBlocker: true }); }} /> : null : null}
    ...
    {dialog?.kind === "delete" && entity ? <DeleteConfirmDialog open onOpenChange={(open) => { if (!open) setDialog(undefined); }} deleteType={level === "graph" ? "Graph Node" : "Action Node"} resourceName={entity.id} onConfirm={() => {
