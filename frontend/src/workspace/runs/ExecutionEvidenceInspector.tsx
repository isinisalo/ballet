import type { ExecutionGraphOccurrenceV2, PolicyProjectionV2 } from "@shared/api/workspace-contracts";
...
export function ExecutionEvidenceInspector({ occurrence, localProjection, onClose }: {
  occurrence: ExecutionGraphOccurrenceV2; localProjection?: PolicyProjectionV2; onClose: () => void;
}) {
  ...
  const body = <ScrollArea className="min-h-0 flex-1"><div className="grid gap-4 p-4">
    ...
    <EvidenceGroup label="Actual cost / duration" value={`${occurrence.actualCostMicros === undefined ? "Not recorded" : formatCost(occurrence.actualCostMicros)} / ${occurrence.durationMillis === undefined ? "Not observed" : formatDuration(occurrence.durationMillis)}`} />
