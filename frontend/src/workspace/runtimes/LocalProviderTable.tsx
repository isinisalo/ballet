...
function ProviderRow({ provider }: { provider: LocalProviderStatus }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell><Badge variant="outline" className={cn("font-mono text-[0.62rem]", toneClass[readiness.tone])}>{readiness.label}</Badge>{provider.healthMessage ? <span className="mt-1 block max-w-64 whitespace-normal text-[0.65rem] text-muted-foreground">{provider.healthMessage}</span> : null}</TableCell>
