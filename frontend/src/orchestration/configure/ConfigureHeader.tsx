...
export function ConfigureHeader({ eyebrow = "Configure", title, description, status, actions }: {
  ...
  return (
    <header className="flex flex-col gap-3 border-b border-border bg-surface px-4 py-5 md:flex-row md:items-end md:justify-between md:px-6">
      ...
      <div className="flex flex-wrap items-center gap-2">{status ? <OperationalStatus label={status} tone={status.includes("Locked") ? "attention" : "healthy"} /> : null}{actions}</div>
    </header>
  );
}
