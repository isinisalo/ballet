...
export function StateWorkspace({ state, locked, navigate, onSave, onMoveAction }: {
  ...
  return <><ConfigureHeader title={state.name} description="State metadata and priority-sorted Actions." status={locked ? "Locked by active Run" : `Order ${state.order}`} actions={<Button variant="outline" onClick={() => navigate("/configure/environment")}><ArrowLeft />Environment</Button>} />
