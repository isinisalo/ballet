...
function ResourceMarkdownEditor({ kind, resource, references, locked, creating, status, onCreate, onDirty, onSave }: {
  ...
  return <div className="min-w-0"><ConfigureToolbar status={status} label={creating ? "New resource" : id}><Button size="sm" variant="outline" disabled={locked} onClick={onCreate}>Create</Button><EditorActions saveLabel="Save Markdown" formId={formId} dirty={dirty} valid={valid} pending={pending} /></ConfigureToolbar><MarkdownConflict stale={editor.stale} hash={resource.contentHash} onReload={editor.reload} /><div className="space-y-3 p-4 md:p-6">{creating ? <label className="grid gap-1 text-sm">Exact resource ID<Input value={id} onChange={(event) => setId(event.target.value)} /></label> : null}<p role={kind === "skills" ? "alert" : undefined} className="text-sm text-muted-foreground">{impact}</p><MarkdownWorkbench document={markdownEntity(resource, { frontmatterText, bodyText })} emptyTitle="Select a Markdown resource" formId={formId} saveLabel="Save Markdown" frontmatterText={frontmatterText} bodyText={bodyText} dirty={dirty} valid={valid} pending={pending} fieldErrors={issues[0] ? { body: `${issues[0].path}: ${issues[0].message}` } : undefined} serverError={error} showActions={false} onFrontmatterChange={setFrontmatterText} onBodyChange={setBodyText} onSubmit={save} /></div></div>;
}
