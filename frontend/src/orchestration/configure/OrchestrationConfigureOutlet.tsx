...
export function OrchestrationConfigureOutlet({ route, data, navigate, mutation }: { route: RouteState; data: OrchestrationConfigureData; navigate: WorkspaceNavigation["navigate"]; mutation: Mutation }) {
  ...
  switch (route.workspaceView) {
    ...
    case "environment": content = <LoopEngineeringWorkspace environment={config.environment} navigate={navigate}><EnvironmentWorkspace environment={config.environment} issues={validateRunnableEnvironment(config.environment)} locked={locked} navigate={navigate} onReorder={(ids) => mutation.run(() => orchestrationApi.reorderStates(ids, configHash))} onSave={async (environment) => { await mutation.run(() => orchestrationApi.saveEnvironment(environment, configHash)); }} onCreate={async (id, name) => { const template = config.environment.states[0]!.actions[0]!; const actionId = `${id}-action-1`; const action: ActionDefinition = { ...template, id: actionId, name: `${name} first Action`, description: `Complete the first bounded Action for ${name}.`, priority: 1, validation: { ...template.validation, agentId: actionAgentId(actionId, "validation") }, work: { ...template.work, agentId: actionAgentId(actionId, "work") } }; const state: StateDefinition = { id, name, description: `${name} completion boundary`, order: config.environment.states.length + 1, actions: [action] }; await mutation.run(() => orchestrationApi.createState(state, configHash)); }} /></LoopEngineeringWorkspace>; break;
    case "state": { const state = config.environment.states.find((item) => item.id === route.stateId); content = <LoopEngineeringWorkspace environment={config.environment} selectedStateId={state?.id} navigate={navigate}><StateWorkspace state={state} locked={locked} navigate={navigate} onReorderActions={(ids) => state ? mutation.run(() => orchestrationApi.reprioritizeActions(state.id, ids, configHash)) : Promise.resolve(false)} onSave={async (next: StateDefinition) => { await mutation.run(() => orchestrationApi.updateState(next, configHash)); }} onDelete={async () => { if (state && await mutation.run(() => orchestrationApi.deleteState(state.id, configHash))) navigate("/automation/loops"); }} onCreateAction={async (id, name) => { if (!state) return; const template = state.actions[0]!; const action: ActionDefinition = { ...template, id, name, description: `${name} bounded work`, priority: state.actions.length + 1, validation: { ...template.validation, agentId: actionAgentId(id, "validation") }, work: { ...template.work, agentId: actionAgentId(id, "work") } }; await mutation.run(() => orchestrationApi.createAction(state.id, action, configHash)); }} /></LoopEngineeringWorkspace>; break; }
    case "action": { const state = config.environment.states.find((item) => item.id === route.stateId); const action = state?.actions.find((item) => item.id === route.actionId); const changeCanvasMode = (mode: "space" | "flow") => { if (!state || !action) return; navigate(mode === "flow" ? orchestrationActionFlowPath(state.id, action.id) : orchestrationActionPath(state.id, action.id), { bypassBlocker: true }); }; content = <LoopEngineeringWorkspace environment={config.environment} selectedStateId={state?.id} selectedActionId={action?.id} action={action} canvasMode={route.canvasMode} navigate={navigate} onActionFlowOpen={(stateId, actionId) => navigate(orchestrationActionFlowPath(stateId, actionId), { bypassBlocker: true })}><ActionWorkspace key={action?.id} stateId={state?.id} action={action} skills={data.skills} locked={locked} canvasMode={route.canvasMode} navigate={navigate} onCanvasModeChange={changeCanvasMode} onDelete={async () => { if (!state || !action) return; await mutation.run(() => orchestrationApi.deleteAction(state.id, action.id, configHash)); navigate(orchestrationStatePath(state.id)); }} onSave={async (next, agents) => { if (state) await mutation.run(() => orchestrationApi.updateAction(state.id, next, configHash, agents)); }} /></LoopEngineeringWorkspace>; break; }
