...
import { orchestrationActionFlowPath } from "@/workspace/routing";
...
export function OrchestrationConfigureOutlet({ route, data, navigate, mutation }: { route: RouteState; data: OrchestrationConfigureData; navigate: WorkspaceNavigation["navigate"]; mutation: Mutation }) {
  ...
  switch (route.workspaceView) {
    ...
    case "state": { const state = config.environment.states.find((item) => item.id === route.stateId); content = <LoopEngineeringWorkspace environment={config.environment} selectedStateId={state?.id} navigate={navigate}><StateWorkspace state={state} locked={locked} navigate={navigate} onReorderActions={(ids) => state ? mutation.run(() => orchestrationApi.reprioritizeActions(state.id, ids, configHash)) : Promise.resolve(false)} onSave={async (next: StateDefinition) => { await mutation.run(() => orchestrationApi.updateState(next, configHash)); }} onDelete={async () => { if (state && await mutation.run(() => orchestrationApi.deleteState(state.id, configHash))) navigate("/automation/loops"); }} onDeleteAction={async (actionId) => { if (state) await mutation.run(() => orchestrationApi.deleteAction(state.id, actionId, configHash)); }} onCreateAction={async (id, name) => { if (!state) return; const template = state.actions[0]!; const action: ActionDefinition = { ...template, id, name, description: `${name} bounded work`, priority: state.actions.length + 1, validation: { ...template.validation, agentId: actionAgentId(id, "validation") }, work: { ...template.work, agentId: actionAgentId(id, "work") } }; await mutation.run(() => orchestrationApi.createAction(state.id, action, configHash)); }} /></LoopEngineeringWorkspace>; break; }
    case "action": { const state = config.environment.states.find((item) => item.id === route.stateId); const action = state?.actions.find((item) => item.id === route.actionId); content = <LoopEngineeringWorkspace environment={config.environment} selectedStateId={state?.id} selectedActionId={action?.id} action={action} canvasMode={route.canvasMode} navigate={navigate} onActionFlowOpen={(stateId, actionId) => navigate(orchestrationActionFlowPath(stateId, actionId), { bypassBlocker: true })}><ActionWorkspace key={action?.id} stateId={state?.id} action={action} skills={data.skills} locked={locked} onSave={async (next, agents) => { if (state) await mutation.run(() => orchestrationApi.updateAction(state.id, next, configHash, agents)); }} /></LoopEngineeringWorkspace>; break; }
