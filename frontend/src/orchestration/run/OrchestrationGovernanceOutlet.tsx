...
export function OrchestrationGovernanceOutlet({ route, configure, governance, navigate, mutation }: { route: RouteState; configure: OrchestrationConfigureData; governance: GovernanceData; navigate(path: string): void; mutation: ReturnType<typeof useOrchestrationMutation> }) {
  ...
  switch (route.workspaceView) {
    ...
    case "run-detail": case "run-state": case "run-action": content = <RunDetailWorkspace run={governance.selectedRun} feedback={governance.feedback} stateId={route.stateId} actionId={route.actionId} navigate={navigate} onCancel={async (id) => { await mutation.run(() => orchestrationApi.cancelRun(id)); }} />; break;
    case "feedback-list": case "feedback-detail": content = <FeedbackWorkspace rows={governance.feedback} runs={governance.runs} selected={governance.selectedFeedback} selectedId={route.workspaceView === "feedback-detail" ? route.entityId : undefined} navigate={navigate} onCreate={async (input) => { await mutation.run(() => orchestrationApi.createFeedback(input)); }} onDecision={async (id, from, decision) => { await mutation.run(() => orchestrationApi.decideFeedback(id, from, decision)); }} onRefinement={async (runId, feedbackId) => { await mutation.run(() => orchestrationApi.createRefinement(runId, [feedbackId])); navigate("/reviews/refinement"); }} />; break;
    ...
    case "products": case "product-detail": content = <ProductWorkspace products={governance.products} selected={governance.selectedProduct} selectedId={route.workspaceView === "product-detail" ? route.entityId : undefined} runs={governance.runs} navigate={navigate} />; break;
...
export const isGovernanceView = (view: RouteState["workspaceView"]) => ["run-list", "run-detail", "run-state", "run-action", "feedback-list", "feedback-detail", "critic-reviews", "critic-proposal", "refinement-reviews", "refinement-proposal", "products", "product-detail"].includes(String(view));
export type { JsonRow };
