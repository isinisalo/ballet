...
export function OrchestrationFrame({ sidebar, children }: { sidebar: ReactNode; children: ReactNode }) {
  return (
    <SidebarProvider>
      {sidebar}
      <div data-slot="sidebar-inset" className="relative flex min-w-0 flex-1 flex-col overflow-hidden bg-background">
        <ScrollArea className="h-svh min-w-0">
          <main className="orchestration-workspace flex min-h-svh min-w-0 flex-col bg-background">
            <header className="p-3 pb-0 md:hidden"><SidebarTrigger className="min-h-10 min-w-10 md:hidden" aria-label="Open navigation" /></header>
            {children}
          </main>
        </ScrollArea>
      </div>
