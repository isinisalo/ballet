...
export function OrchestrationWorkspaceShell({ route, navigate, setNavigationBlocker }: { route: RouteState; navigate: WorkspaceNavigation["navigate"]; setNavigationBlocker: WorkspaceNavigation["setNavigationBlocker"] }) {
  ...
  else if (!configure.loading && configure.data && isGovernanceView(route.workspaceView) && !governance.loading && governance.data) content = <div onInput={() => setDirty(true)}><OrchestrationGovernanceOutlet route={route} configure={configure.data} governance={governance.data} navigate={navigate} mutation={mutation} /></div>;
  else if (!configure.loading && configure.data && !isGovernanceView(route.workspaceView)) content = <div onInput={() => setDirty(true)}><OrchestrationConfigureOutlet route={route} data={configure.data} navigate={navigate} mutation={mutation} /></div>;
  return <OrchestrationFrame sidebar={<OrchestrationSidebar route={route} navigate={navigate} />}>{content}</OrchestrationFrame>;
}
