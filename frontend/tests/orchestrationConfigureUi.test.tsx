...
describe("orchestration Configure UI", () => {
  ...
  it("presents Validation as main and Work as subordinate", async () => { renderAction(); expect(await screen.findByText("Validation Agent · main/controller")).toBeInTheDocument(); expect(screen.getByText("Work Agent · subordinate")).toBeInTheDocument(); });
  ...
  it("opens and closes the Action flow with an accessible control", async () => { mockActionAgents(); const user = userEvent.setup(); const changeMode = vi.fn(); const config = orchestrationConfig(); const props = { stateId: "state-1", action: config.environment.states[0]!.actions[0], skills: resources().filter((item) => item.kind === "skill"), locked: false, navigate: vi.fn(), onCanvasModeChange: changeMode, onSave: vi.fn() }; const { rerender } = render(<ActionWorkspace {...props} />); await user.click(screen.getByRole("button", { name: "Open Action flow" })); expect(changeMode).toHaveBeenCalledWith("flow"); await user.type(screen.getByLabelText("Name"), " draft"); rerender(<ActionWorkspace {...props} canvasMode="flow" />); expect(screen.getByLabelText("Name")).toHaveValue(`${props.action.name} draft`); await user.click(screen.getByRole("button", { name: "Show space canvas" })); expect(changeMode).toHaveBeenLastCalledWith("space"); });
  ...
  it("keeps Action execution Codex-only with TOML model, reasoning, instructions and Skills", async () => { renderAction(); expect(screen.getByLabelText("maxRetries")).toHaveAttribute("type", "number"); const validation = await screen.findByRole("group", { name: "Validation Agent · main/controller" }); expect(screen.queryByLabelText("Provider")).not.toBeInTheDocument(); expect(screen.queryByRole("switch", { name: "Network" })).not.toBeInTheDocument(); expect(within(validation).getByLabelText("Model")).toBeInTheDocument(); expect(within(validation).getByLabelText("validation developer instructions")).toBeInTheDocument(); expect(within(validation).getByLabelText("validation Skills")).toBeInTheDocument(); expect(screen.queryByText("Save execution")).not.toBeInTheDocument(); });
  it("saves Project Config and both Action Agent TOMLs through one Action command", async () => {
    const user = userEvent.setup(); mockActionAgents(); const save = vi.fn(); renderActionWorkspace(false, save);
  ...
  it("reports missing Action Agent TOMLs in Run readiness", async () => { mockActionAgents({ missing: true }); renderActionWorkspace(); await waitFor(() => expect(screen.getByText("Action Agent definitions are unavailable.")).toBeInTheDocument()); expect(screen.getByRole("toolbar")).toHaveTextContent("Not ready"); });
});

function renderAction(locked = false) { mockActionAgents(); renderActionWorkspace(locked); }

function renderActionWorkspace(locked = false, onSave = vi.fn()) { const config = orchestrationConfig(); render(<ActionWorkspace stateId="state-1" action={config.environment.states[0]!.actions[0]} skills={resources().filter((item) => item.kind === "skill")} locked={locked} navigate={vi.fn()} onCanvasModeChange={vi.fn()} onSave={onSave} />); }

function mockActionAgents(options: { duplicate?: boolean; missing?: boolean } = {}) {
  ...
  const agent = (role: "validation" | "work", hash: string) => ({ id: action[role].agentId,
    ...
    agent: { id: action[role].agentId, name: action[role].agentId, description: `${role} Agent`,
      developerInstructions: options.duplicate ? "same instructions" : `${role} action-1 instructions`, model: "gpt", reasoningEffort: "medium" } });
