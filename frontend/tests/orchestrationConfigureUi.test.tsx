...
describe("orchestration Configure UI", () => {
  ...
  it("shows one local daemon and disables restart while work is active", async () => { vi.spyOn(orchestrationApi, "localRuntime").mockResolvedValue({ status: "online", daemonVersion: "2", uptimeSeconds: 12, activeTaskCount: 1, lastSeenAt: new Date().toISOString(), refreshRequested: false, restartRequested: false, providers: [{ provider: "codex", health: "ready", authStatus: "authenticated", busy: true, checkedAt: new Date().toISOString(), capabilities: { models: [{ id: "gpt", reasoningOptions: ["medium"] }], policy: { networkControl: true, readOnlyRoots: true } } }] }); render(<RuntimesWorkspace navigate={vi.fn()} />); expect(await screen.findByRole("heading", { name: "Checkout-local daemon" })).toBeInTheDocument(); expect(screen.queryByText(/pair/i)).not.toBeInTheDocument(); expect(screen.getByRole("button", { name: "Restart daemon" })).toBeDisabled(); expect(screen.getByText("Codex CLI")).toBeInTheDocument(); });
