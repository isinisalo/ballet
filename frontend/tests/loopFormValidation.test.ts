...
describe("Loop form validation", () => {
  it("uses the shared automation schema and rejects duplicate entity ids", () => {
    const valid: ProjectAutomationConfig = { version: 6, loops: [loop()] };
    ...
    expect(automationDraftIsValid({ version: 6, loops: [duplicateSteps] })).toBe(false);
  });

  it("provides adjacent Loop and Step field messages", () => {
    const current = loop();
    expect(loopIdError({ ...current, id: "Invalid ID" }, [])).toBe("Use 1–160 lowercase kebab-case characters.");
