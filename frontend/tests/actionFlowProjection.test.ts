...
import type { ProjectActionNode } from "@shared/api/workspace-contracts";
import { actionFlowLayout, projectActionFlow } from "../src/workspace/automation/actionFlowProjection";

describe("Action flow projection", () => {
  it.each(["wide", "narrow"] as const)("produces a deterministic %s layout", (viewport) => {
    const first = actionFlowLayout(viewport, true);
    expect(actionFlowLayout(viewport, true)).toEqual(first);
    expect(first.points.retryCount).toBeDefined();
    expect(first.points.continue).toBeDefined();
    expect(projectActionFlow(actionNode(2)).retryCount).toBe(2);
  ...
  it("removes the retry return route when maxRetries is zero", () => {
    const projection = projectActionFlow(actionNode(0));
    const layout = actionFlowLayout("wide", projection.retryEnabled);
...
const actionNode = (maxRetries: number): ProjectActionNode => ({
