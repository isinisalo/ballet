...
describe("orchestration Run and governance models", () => {
  ...
  it("binds Critic approval to proposal hash and lets the server derive its immutable Run owner", () => expect(criticDecisionRequest({ critic_proposal_id: "critic-1", content_hash: "hash", content_json: JSON.stringify({ title: "Finding", finding: "Detail", recommendedCorrectiveActions: ["Fix"], evidenceRefs: ["evidence"] }) }, "approved")).toEqual({ decision: "approved", expectedContentHash: "hash", expectedVersion: 1, feedback: { feedbackEntryId: "feedback-critic-1", title: "Finding", description: "Detail", correctiveActions: ["Fix"], evidenceRefs: ["evidence"] } }));
