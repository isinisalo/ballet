...
import {
  ...
  statusPresentation,
  timelineForAction
} from "../src/vnext/runModels";
import type { RunDetail, RunSummary, StateProjection } from "../src/vnext/runTypes";
...
describe("vNext Run and governance models", () => {
  ...
  it("binds Critic approval to proposal hash and one exact Feedback id", () => expect(criticDecisionRequest({ critic_proposal_id: "critic-1", content_hash: "hash", content_json: JSON.stringify({ title: "Finding", finding: "Detail", recommendedCorrectiveActions: ["Fix"], evidenceRefs: ["evidence"] }) }, "approved", "run-1")).toEqual({ decision: "approved", expectedContentHash: "hash", expectedVersion: 1, feedback: { feedbackEntryId: "feedback-critic-1", environmentRunId: "run-1", title: "Finding", description: "Detail", correctiveActions: ["Fix"], evidenceRefs: ["evidence"] } }));
  it("binds Refinement approval to exact proposal, file, and Action hashes", () => expect(refinementDecisionRequest({ change_list_hash: "list", impact_scope_json: JSON.stringify({ actionIds: ["b", "a"] }), files: [{ proposed_content_hash: "z" }, { proposed_content_hash: "a" }] }, "approved")).toEqual({ decision: "approved", expectedContentHash: "list", expectedVersion: 1, expectedChangeHashes: ["a", "z"], expectedImpactActionIds: ["a", "b"], acknowledgeLocalCommitAndContinuation: true }));
  it("provides a screen-reader textual marker for every changed line", () => expect(diffPresentation({ operation: "update", relative_path: "a.md", expected_preimage_hash: "old", proposed_content_hash: "new", proposed_content: "one\ntwo" }).lines).toEqual([{ line: 1, marker: "+", text: "one" }, { line: 2, marker: "+", text: "two" }]));
