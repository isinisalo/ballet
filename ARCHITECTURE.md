...
| Action execution binding / Agent binding / Run Evidence | 2 / 2 / 1 |
...
flowchart LR
  ...
  Ballet -->|runtime facts| DB[(SQLite v20)]
...
flowchart TB
  ...
  App --> LocalRuntime[Action execution and governance Agent bindings, queue, leases and local runtime registry]
...
| Run planning | approved State closure, immutable Snapshot v17, Action execution bindings, ordered State/Action seeds and permissions | `backend/orchestration/runtime/EnvironmentRunPlanner.ts` |
...
| Persistence | SQLite v20 schema, Action execution bindings, transactions, events, daemon facts, Feedback, reviews and Run Evidence | `backend/orchestration/persistence/**` |
...
| Runtime status, attempts, gates, schedules, Action execution/governance Agent bindings and decisions | SQLite v20 plus immutable Root Snapshot | config `done`/`blocked` fields or provider prose |
...
sequenceDiagram
  ...
  A->>DB: record commit and create one continuation Snapshot v17
...
- Dispatch specs and prompt evidence use a discriminated `action_role | agent` subject. The immutable Action capability stores its shared provider and policy once with role-specific model/reasoning capability evidence. Validation, Critic and Refinement proposal are read-only; Work writes only within its managed worktree; approval policy is always `never`.
- Validation and Work select one shared provider and network/read-only-root policy in the machine-local Action execution binding, with separate role models and reasoning. Critic/Refinement continue to use Agent bindings from checkout-local daemon capabilities.
